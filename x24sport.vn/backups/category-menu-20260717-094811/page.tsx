    ...
    <footer className="store-footer" id="lien-he">
      <div className="site-container">
        ...
        <div className="footer-main">
          <div className="footer-about"><Logo /><p>X24Sport cung cấp trang phục thể thao thiết kế theo yêu cầu cho đội bóng, câu lạc bộ và cộng đồng vận động.</p><a href="tel:0989353247"><Phone size={18} /> 0989 353 247</a></div>
          <div><h3>Danh mục</h3>{categories.slice(0, 4).map((category) => <Link href={`/danh-muc/${category.slug}`} key={category.slug}>Áo {category.name}</Link>)}</div>
          <div><h3>Khám phá</h3>{categories.slice(4).map((category) => <Link href={`/danh-muc/${category.slug}`} key={category.slug}>Áo {category.name}</Link>)}<Link href="/san-pham">Tất cả sản phẩm</Link><Link href="/blog/">Blog thể thao</Link></div>
        ...
        <section className="hero-directory" id="bo-mon">
          ...
          <div className="directory-panel">
            ...
            <div className="category-grid">
              {categories.map((category) => <Link href={`/danh-muc/${category.slug}`} key={category.slug}><Image src={category.image} alt="" fill sizes="(max-width: 760px) 31vw, 12vw" /><span>ÁO {category.name.toUpperCase()}</span></Link>)}
            </div>
          </div>
        </section>
