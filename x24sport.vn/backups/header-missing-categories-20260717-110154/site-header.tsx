        ...
        <nav className="nav-bar" aria-label="Điều hướng chính">
          <div className="site-container">
            ...
            <div className="nav-dropdown">
              <Link className="nav-trigger" href="/san-pham/">Sản phẩm</Link>
              <div className="nav-mega" role="menu">
                <div className="nav-mega-heading">
                  <span>Danh mục X24Sport</span>
                  <strong>Chọn theo bộ môn</strong>
                </div>
                <div className="nav-mega-grid">
                  {categoryMenu.map((group) => (
                    <section key={group.slug}>
                      <Link className="nav-group-link" href={`/danh-muc/${group.slug}`}>{group.name}</Link>
                      {group.children.map((child) => <Link href={`/danh-muc/${child.slug}`} key={child.slug}>{child.name}</Link>)}
                    </section>
                  ))}
                </div>
            ...
            {categories.slice(0, 6).map((category) => <Link href={`/danh-muc/${category.slug}`} key={category.slug}>{category.name}</Link>)}
            <Link href="/blog/">Blog</Link>
