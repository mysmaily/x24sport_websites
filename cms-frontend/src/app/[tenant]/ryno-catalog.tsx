...
export async function RynoProductsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <div className="ryno-store"><RynoSiteHeader /><RynoCatalogBody title="Sản phẩm RynoSport" description="Khám phá trang phục thể thao dành cho đội nhóm, câu lạc bộ và những người luôn muốn tiến lên." categories={categories} products={products} /><RynoSiteFooter /></div>
}
...
export async function RynoCategoryPage({ slug }: { slug: string }) {
  ...
  return <div className="ryno-store"><RynoSiteHeader /><RynoCatalogBody title={category.name} description={category.description || `Mẫu trang phục ${category.name.toLowerCase()} được chọn cho đội hình của bạn.`} categories={categories} products={products} activeSlug={slug} /><RynoSiteFooter /></div>
}
