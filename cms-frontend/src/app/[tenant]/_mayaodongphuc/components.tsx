...
export function UniformProductCard({ product, eager = false }: { product: UniformProduct; eager?: boolean }) {
  ...
  return <article className={styles.productCard}><Link className={styles.productImage} href={`/san-pham/${product.slug}/`}>{image?.url ? <Image alt={image.alt || product.name} fill loading={eager ? 'eager' : 'lazy'} fetchPriority={eager ? 'high' : 'auto'} sizes="(max-width: 640px) 50vw, (max-width: 1000px) 33vw, 25vw" src={image.url} /> : null}{badge ? <span>{badge}</span> : null}<i><ArrowRight /></i></Link><div className={styles.productMeta}><p><span>{product.sku || 'MẪU ĐỒNG PHỤC'}</span><span>{productMaterial(product)}</span></p><h3><Link href={`/san-pham/${product.slug}/`}>{product.name}</Link></h3><div><b>Báo giá theo yêu cầu</b><span className={styles.swatches} aria-label="Bảng màu gợi ý">{productColors(product).map((color) => <i key={color} style={{ backgroundColor: color }} />)}</span></div>{category ? <Link className={styles.cardCategoryLink} href={`/danh-muc/${category.slug}/`}>{category.name}</Link> : null}</div></article>
}
