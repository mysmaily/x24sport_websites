...
export async function MayAoDongPhucHome() {
  ...
  return <MayAoDongPhucShell>
    ...
    <section className={styles.hero}>
      ...
      <aside className={styles.finder}><span>01 / BẮT ĐẦU NHANH</span><h2>Bạn đang cần may cho?</h2><div className={styles.finderLinks}>{categories.slice(0, 4).map((item) => <Link href={`/danh-muc/${item.slug}/`} key={item.slug}><b>{item.name}</b><small>{uniformPublicCopy(item.description)}</small><ArrowRight aria-hidden="true" /></Link>)}</div><Link href="/san-pham/">Xem toàn bộ catalog <ArrowRight aria-hidden="true" /></Link></aside>
      <div className={styles.materialBento}><span>02 / VẬT LIỆU</span><Layers3 aria-hidden="true" /><h2>Chọn theo nhịp làm việc</h2><p>Độ thoáng, giữ form và độ bền được cân bằng cho từng bối cảnh.</p><Link href="#vat-lieu">Mở thư viện vật liệu <ArrowRight aria-hidden="true" /></Link></div>
    </section>
    ...
    <section className={styles.process} id="quy-trinh"><div className={styles.processTitle}><span>04 / WORKFLOW</span><h2>Một đường chạy.<br />Bốn điểm duyệt.</h2><p>Người đặt hàng luôn biết đang ở đâu và cần xác nhận điều gì tiếp theo.</p></div><ol>{process.map(({ icon: Icon, step, title, text }) => <li key={step}><span>{step}</span><Icon aria-hidden="true" /><div><h3>{title}</h3><p>{text}</p></div></li>)}</ol></section>
