...
export default async function SearchPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <main>
      ...
      <section className="px-[clamp(20px,5vw,76px)] py-7">
        <p className="mb-3 text-xs font-black uppercase text-[var(--accent)]">Tìm kiếm sản phẩm</p>
        <h1 className="max-w-[980px] text-[20px] font-black leading-[1.2] md:text-[22px]">{query ? `Kết quả tìm kiếm: ${query}` : 'Tìm mẫu áo bóng chuyền'}</h1>
        <form action="/tim-kiem/" className="mt-5 grid max-w-3xl grid-cols-[auto_1fr_auto] overflow-hidden border border-[var(--line)] bg-white" role="search">
      ...
      <section className="grid gap-5 px-[clamp(20px,5vw,76px)] pb-[72px] md:grid-cols-2 xl:grid-cols-3">
        {products.map((product, index) => (
          <article className="mbc-product-card border border-[var(--line)] bg-white/6" key={product.id}>
            ...
            <div className="p-[22px]"><p className="mb-2 text-xs font-black text-[var(--accent)]">{product.sku}</p><h2 className="mb-2.5 text-[18px]"><a href={productHref(product)}>{product.name}</a></h2><span className="leading-[1.55] text-[var(--muted)]">{product.shortDescription}</span><strong className="mt-4 block text-2xl">{formatPrice(product.price)}</strong></div>
          </article>
        ))}
