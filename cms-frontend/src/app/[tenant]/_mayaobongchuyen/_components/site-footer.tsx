  ...
  return (
    <>
        ...
        <div className="mx-auto grid max-w-[1360px] gap-10 md:grid-cols-2 lg:grid-cols-[1.25fr_1fr_1fr_1fr]">
          ...
          <div>
            ...
            <div className="grid gap-3 text-sm text-[var(--muted)]">
              <a className="inline-flex items-center gap-2" href={`tel:${PHONE_VALUE}`}><Phone aria-hidden="true" size={16} /> {PHONE_DISPLAY}</a>
              <a className="inline-flex items-center gap-2" href={ZALO_URL} rel="noreferrer" target="_blank"><MessageCircle aria-hidden="true" size={16} /> Zalo tư vấn</a>
            </div>
          </div>
        </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-2 border-t border-white/10 bg-[#05070c] p-2 lg:hidden">
        <a className="inline-flex min-h-12 items-center justify-center gap-2 bg-white text-sm font-black text-[#080b12]" href={`tel:${PHONE_VALUE}`}><Phone aria-hidden="true" size={17} /> Gọi ngay</a>
        <a className="inline-flex min-h-12 items-center justify-center gap-2 bg-[var(--accent)] text-sm font-black text-white" href={ZALO_URL} rel="noreferrer" target="_blank"><MessageCircle aria-hidden="true" size={17} /> Zalo</a>
      </div>
    </>
  )
}
