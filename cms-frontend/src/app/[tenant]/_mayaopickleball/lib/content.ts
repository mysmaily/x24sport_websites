...
export const PRODUCT_PROMOTION = {
  price: 129000,
...
const fallbackProducts: Product[] = [
  { id: '1', name: 'Áo pickleball Paddle Court xanh lá trắng', slug: 'paddle-court-jersey', sku: 'MP-PADDLE-01', price: 129000, compareAtPrice: 159000, shortDescription: 'Áo pickleball cổ tròn, vải nhẹ, phối màu mạnh cho đội thi đấu.', badges: [{ label: 'Đặt may' }] },
  { id: '2', name: 'Polo pickleball Dink Pro xanh navy', slug: 'dink-pro-polo', sku: 'MP-DINK-02', price: 129000, compareAtPrice: 159000, shortDescription: 'Polo thi đấu mặt chim, cổ đứng, phù hợp in tên số và logo CLB.', badges: [{ label: 'Polo' }] },
  { id: '3', name: 'Set đồng phục pickleball Kitchen Team', slug: 'kitchen-team-set', sku: 'MP-KITCHEN-03', price: 129000, compareAtPrice: 159000, shortDescription: 'Bộ đồng phục pickleball nam nữ cho giải phong trào và câu lạc bộ.', badges: [{ label: 'Team set' }] },
]

function withProductPromotion<T extends Product>(product: T): T {
  return {
    ...product,
    price: PRODUCT_PROMOTION.price,
    compareAtPrice: PRODUCT_PROMOTION.compareAtPrice,
  }
}
...
export async function getHomeData() {
  ...
  try {
    ...
    return {
      ...
      products: (products.length ? products : fallbackProducts).map(withProductPromotion),
...
export async function getPostsPage(page = 1, perPage = POSTS_PER_PAGE) {
  ...
  try {
    ...
    const data = await fetchDocsPaginated<Post>(`/api/posts?${params.toString()}`)
    if (!data) return { docs: fallbackPosts, totalDocs: fallbackPosts.length, totalPages: 1, page: 1 }

    return {
      docs: data.docs.length ? data.docs : fallbackPosts,
      totalDocs: data.totalDocs || fallbackPosts.length,
...
export async function getFinishedSamplePostsPage(page = 1, perPage = POSTS_PER_PAGE) {
  const tenantSlug = await getTenantSlug()
  const requestedPage = Math.max(1, page)
  ...
  try {
    ...
    const data = await fetchDocsPaginated<Post>(`/api/posts?${params.toString()}`)
    const samples = (data?.docs || []).filter(isFinishedSamplePost)
    return {
      docs: samples.slice((requestedPage - 1) * perPage, requestedPage * perPage),
      totalDocs: samples.length,
      totalPages: Math.max(1, Math.ceil(samples.length / perPage)),
      page: requestedPage,
    }
  } catch {
...
export async function getAllProducts(page = 1, perPage = PRODUCTS_PER_PAGE): Promise<PaginatedProducts> {
  ...
  try {
    ...
    return {
      products: (data.docs.length ? data.docs : fallbackProducts.slice(0, perPage)).map(withProductPromotion),
...
export async function searchProducts(search: string, page = 1, perPage = PRODUCTS_PER_PAGE): Promise<PaginatedProducts> {
  ...
  try {
    ...
    return {
      products: data.docs.map(withProductPromotion),
      totalDocs: data.totalDocs,
...
export async function getProductsByCatalogFilter(
  ...
  try {
    ...
    if (mediaData && mediaData.docs.length) {
      return {
        products: mediaData.docs.map(withProductPromotion),
    ...
    if (productData && productData.docs.length) {
      return {
        products: productData.docs.map(withProductPromotion),
...
export async function getProductBySlug(slug: string) {
  ...
  try {
    ...
    return product ? withProductPromotion(product) : fallbackProduct ? withProductPromotion(fallbackProduct) : null
  } catch {
