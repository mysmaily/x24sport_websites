...
export default async function HomePage() {
  const [catalog, hotCatalog, posts, categoryResult] = await Promise.all([getProducts({ limit: 8 }), getProducts({ limit: 4, sort: 'popular' }), getLatestPosts(3), getCategories()])
  ...
  return <>
    ...
    <section className="mcb-hot-products-section py-16 text-white sm:py-22"><div className="section-shell"><div className="mcb-hot-products-panel rounded-[1.35rem] p-4 sm:p-6 lg:p-8"><div className="max-w-3xl"><p className="section-kicker text-orange-300">Đang được quan tâm</p><h2 className="section-title text-white">Sản phẩm hot được xem nhiều.</h2><p className="section-lead text-slate-300">Những mẫu áo chạy bộ khách thường mở để tham khảo phối màu, kiểu áo và tinh thần thiết kế cho đội.</p></div><div className="mt-8"><ProductGrid products={hotCatalog.docs} /></div><div className="mcb-section-footer-action"><Link className="mcb-light-button inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-white px-5 text-sm font-black text-slate-950 hover:bg-orange-50 hover:text-brand" href="/mau-ao-chay-bo-duoc-xem-nhieu/">Xem tất cả mẫu hot <ArrowRight size={18} /></Link></div></div></div></section>

    <section className="mcb-product-showcase bg-white py-16 sm:py-22"><div className="section-shell"><div className="max-w-3xl"><p className="section-kicker">Mới cập nhật</p><h2 className="section-title">Mẫu mới ra.</h2><p className="section-lead">Các thiết kế vừa cập nhật để đội dễ chọn form áo, phối màu và ý tưởng in ấn cho đơn tiếp theo.</p></div><div className="mt-10"><ProductGrid products={catalog.docs} /></div><div className="mcb-section-footer-action"><Link className="mcb-outline-button inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-slate-300 px-5 text-sm font-black hover:border-brand hover:text-brand" href="/san-pham/">Xem tất cả {catalog.totalDocs.toLocaleString('vi-VN')} mẫu <ArrowRight size={18} /></Link></div></div></section>
