...
import { getCategories, getCategory, getCategoryNavigation, getProductsPage } from '../../../lib/content'
...
export async function PndCatalogPage({ categorySlug, search }: { categorySlug?: string; search: Search }) {
  ...
  const [category, categories, navigation, result] = await Promise.all([
  ...
  const parentCategory = category ? navigation.find((item) => item.children.some((child) => child.slug === category.slug)) : undefined
  const filterCategories = parentCategory?.children || categories.slice(0, 8)
  const allHref = parentCategory ? `/danh-muc/${parentCategory.slug}/` : '/san-pham/'
  const allLabel = parentCategory ? `Tất cả ${parentCategory.name}` : 'Tất cả mẫu'
