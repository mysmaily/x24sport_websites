...
const landing = getFootballAudienceLanding('thiet-ke-ao-bong-da-ngan-hang')
...
export default async function BankFootballLanding({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const query = await searchParams
  const page = Math.max(1, Number(Array.isArray(query.page) ? query.page[0] : query.page) || 1)
