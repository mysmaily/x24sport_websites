...
export function ProductGrid({ products, priorityImages = true }: { products: Product[]; priorityImages?: boolean }) {
  ...
  return <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3 xl:grid-cols-4">{products.map((product, index) => <ProductCard index={priorityImages ? index : 99} key={product.id} product={product} />)}</div>
}
