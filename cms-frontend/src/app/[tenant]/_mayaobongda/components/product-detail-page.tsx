...
function ContextualProductImages({ images, productName }: { images: ReturnType<typeof productImages>; productName: string }) {
  if (!images.length) return null

  return (
    <div className="not-prose mt-9 grid gap-7">
      {images.map((image) => {
        const caption = image.alt?.trim() || `${productName} trong bộ sưu tập mẫu đặt may.`
        return (
          <figure className="overflow-hidden rounded-lg border border-slate-200 bg-white" key={image.id || image.url}>
            <img
              alt={caption}
            ...
            />
            <figcaption className="border-t border-slate-100 px-4 py-3 text-sm leading-6 text-slate-600">{caption}</figcaption>
          </figure>
        )
      })}
    </div>
...
export async function ProductDetailPage({
  ...
  const hasDetailedDescription = Boolean(product.contentHtml || descriptionBlocks.length || contextualImages.length)
      ...
      <article className="section-shell pb-16 sm:pb-22">
        ...
        <div className="grid overflow-hidden rounded-3xl border border-slate-200 bg-white lg:grid-cols-[1.15fr_.85fr]">
          ...
          <div className="flex flex-col p-5 sm:p-7 lg:p-8 xl:p-10">
            <p className="section-kicker">{isLogo ? 'Thiết kế theo nhận diện riêng' : 'Mẫu áo bóng đá'}</p>
            {product.shortDescription ? <p className="text-base leading-7 text-slate-600">{product.shortDescription}</p> : null}
            {hasPrice ? <p className="mt-4 text-sm font-bold text-slate-600">Giá tham khảo: <strong className="text-xl text-brand">{product.price!.toLocaleString('vi-VN')} VND</strong></p> : null}

            <div className="mt-6 grid gap-2 rounded-2xl bg-orange-50 p-4 text-sm text-slate-700">
              <b className="text-base text-slate-950">Có thể chỉnh theo yêu cầu của đội</b>
              <p>Trao đổi mẫu áo, logo, tên số và chi tiết nhận diện trước khi xác nhận đặt may.</p>
              <ul className="grid gap-2 font-bold sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                {['Đổi màu theo đội', 'Thêm logo riêng', 'In tên & số áo', 'Tư vấn size'].map((item) => <li className="flex items-center gap-2" key={item}><span className="size-1.5 rounded-full bg-brand" /> {item}</li>)}
              </ul>
            </div>
            ...
            {showInterestForm ? <ProductInterestForm productName={product.name} productUrl={canonical(productHref)} variant="utility" /> : null}
          </div>
        </div>
        ...
        {hasDetailedDescription ? (
          <section className="grid gap-8 py-14 sm:py-20 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-16">
            ...
            {product.contentHtml ? (
              <div className="prose">
                <div dangerouslySetInnerHTML={{ __html: rewriteLegacyHtml(product.contentHtml) }} />
                <ContextualProductImages images={contextualImages} productName={product.name} />
              </div>
            ) : (
              <div className="prose">
