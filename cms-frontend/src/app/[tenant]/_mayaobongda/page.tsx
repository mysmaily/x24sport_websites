...
export default async function HomePage() {
  ...
  return <>
    ...
    <section className="mabd-home-hero relative overflow-hidden bg-[#0b1220] text-white">
      ...
      <div className="section-shell relative z-10 grid min-h-[560px] items-center gap-7 py-8 sm:min-h-[620px] sm:py-12 lg:min-h-[720px] lg:grid-cols-[.94fr_1.06fr] lg:py-16 xl:gap-14">
        <div className="z-10 min-w-0 max-w-3xl">
          ...
          <ul className="mabd-audience-grid mt-4 grid grid-cols-3 gap-1.5 sm:mt-6 sm:gap-2" aria-label="Doi tuong khach hang chinh">
            {audiences.map(({ href, icon: Icon, label, text }) => <li className="min-w-0" key={label}><Link className="mabd-audience-card group block h-full rounded-lg border border-white/10 bg-white/[.055] p-2.5 backdrop-blur transition duration-200 hover:-translate-y-0.5 hover:border-brand/50 hover:bg-white/[.085] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand sm:rounded-xl sm:p-3.5" href={href}><Icon className="text-brand" size={20} /><strong className="mabd-audience-title mt-2 flex items-center gap-1 text-[11px] font-black leading-4 text-white sm:mt-3 sm:text-sm">{label}<ArrowRight className="hidden opacity-0 transition group-hover:translate-x-0.5 group-hover:opacity-100 sm:block" size={14} /></strong><span className="mabd-audience-desc mt-1 hidden text-[11px] leading-4 text-slate-400 sm:block">{text}</span></Link></li>)}
          </ul>

          <div className="mabd-hero-actions mt-5 flex flex-wrap gap-2 sm:mt-7 sm:gap-3"><Link className="mabd-hero-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-brand px-4 text-sm font-black transition duration-200 hover:bg-brand-dark sm:min-h-13 sm:px-6" href="/san-pham/">Khám phá mẫu áo <ArrowRight size={19} /></Link><a className="mabd-hero-secondary inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-white/25 px-4 text-sm font-black transition duration-200 hover:border-white/50 hover:bg-white/10 sm:min-h-13 sm:px-6" href={ZALO_URL} rel="noreferrer" target="_blank">Tư vấn thiết kế</a></div>
        </div>
      </div>
      <PromoHeroSlider />
    </section>
    ...
    <section className="mabd-category-section section-shell py-10 sm:py-18"><div className="max-w-3xl"><p className="section-kicker">Chọn điểm xuất phát</p><h2 className="section-title">Mẫu áo cho từng kiểu đội hình.</h2><p className="section-lead">Duyệt nhanh theo nhóm sản phẩm. Mỗi mẫu đều có thể chỉnh màu, logo và tên số.</p></div><div className="mabd-category-grid mt-5 grid grid-cols-2 gap-2.5 sm:mt-9 sm:gap-4 lg:grid-cols-3">{categories.map((category, index) => <Link className={`mabd-category-card group relative min-h-36 overflow-hidden rounded-xl p-4 text-white sm:min-h-56 sm:rounded-2xl sm:p-6 ${index === 0 ? 'mabd-category-card-primary bg-brand' : 'mabd-category-card-dark bg-[#0b1220]'}`} href={category.legacyPath || `/${category.slug}/`} key={category.id}><span className="mabd-category-number absolute -bottom-8 -right-4 font-display text-[6rem] font-black leading-none sm:-bottom-12 sm:-right-5 sm:text-[10rem]">0{index + 1}</span><Flag className="mabd-category-icon" size={24} /><h3 className="mabd-category-title mt-8 max-w-sm font-display text-2xl font-bold leading-none sm:mt-16 sm:text-4xl">{category.name}</h3><span className="mabd-category-action mt-3 inline-flex items-center gap-1.5 text-xs font-black sm:mt-5 sm:text-sm">Xem mẫu <ArrowRight size={16} /></span></Link>)}</div></section>
