...
const landing = getFootballAudienceLanding('ao-bong-da-giai-phong-trao')
...
export default async function TournamentLanding({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const query = await searchParams
  const page = Math.max(1, Number(Array.isArray(query.page) ? query.page[0] : query.page) || 1)
