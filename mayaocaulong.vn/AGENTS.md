...
```bash
ssh root@10.10.0.28 'ts=$(date +%Y%m%d-%H%M%S); mkdir -p /root/sports-cms/backups; tar -C /opt -czf /root/sports-cms/backups/mayaocaulong-before-change-$ts.tgz sports-cms/mayaocaulong.vn sports-cms/docker-compose.yml'
```
...
rsync -az --delete \
  ...
  --exclude .env \
  --exclude .env.local \
  mayaocaulong.vn/ root@10.10.0.28:/opt/sports-cms/mayaocaulong.vn/

ssh root@10.10.0.28 'cd /opt/sports-cms && docker compose up -d --build mayaocaulong'
```
...
curl -I http://10.10.0.28:3002
curl -I https://mayaocaulong.vn
ssh root@10.10.0.28 'cd /opt/sports-cms && docker compose logs --tail=120 mayaocaulong'
```
