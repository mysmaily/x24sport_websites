...
def product_payload(item: dict[str, Any], media_id: int, tenant_id: str, runner: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "description": runner.lexical_doc([
...
def main() -> int:
    ...
    if len(catalog) != 3:
        raise RuntimeError("catalog must contain exactly 3 products")

    query = urllib.parse.urlencode({"where[tenant][equals]": tenant_id, "limit": 100})
    current = client.request(f"/api/products?{query}").get("docs", [])
    current_skus = {doc.get("sku") for doc in current}
    requested_skus = {item["code"] for item in catalog}
    duplicates = sorted(current_skus & requested_skus)
    if duplicates:
        raise RuntimeError(f"refusing duplicate SKUs: {duplicates}")
    ...
    try:
        ...
        for row in reversed(created):
            try:
                client.request(f"/api/products/{row['product_id']}", method="DELETE")
            except Exception:
                pass
        raise

    result = {"uploaded_media": uploaded, "created_products": created}
