...
COLLAR_RENDERING_RULES = """Collar rendering rules:
- "Cổ tròn": simple round crew neck, no fold, no buttons.
- "Cổ V viền": standard V-neck with a clean rib/trim border, no fold, no buttons.
- "Cổ V chéo": crossed V-neck wrap/crossover trim only, no folded shirt collar, no buttons.
- "Cổ V phối": contrast-color V-neck trim/panel only. It must NOT look like a folded collar, lapel, shawl collar, or polo collar. No buttons.
- "Cổ polo": the only folded collar option. It must include a visible short button placket with 2-3 buttons below the collar.
Do not create a folded/crossover collar without buttons. If a collar folds down, it must be the "Cổ polo" option and must show buttons."""
...
Asset type: square e-commerce football kit sales visual generated from a product reference image
Input images:
- Source apparel image: {source_image}
- Random chest logo reference: {chest_logo}

Primary request: Make a premium X24Sport football kit sales image from the source. If the source is already a high-quality football-kit lifestyle/photo image, use source-photo-first mode: preserve the original person, pose, face, kit proportions, field/stadium background, lighting, depth of field, and color mood as much as possible. Do not rebuild the scene, do not replace the model, and do not turn it into a heavy split catalog layout. Only add tasteful collar options and contact/footer text in open space.
Sport conversion: if the source comes from basketball, pickleball, badminton, running, or another sport, treat it as an apparel-design reference and convert the context to football. Preserve the shirt design accurately, but remove or convert non-football sport details. Basketball balls become football/soccer balls; basketball courts, hoops, paddles, rackets, nets, and sport-specific props become football pitch/stadium or neutral studio details. Do not keep basketball/pickleball/badminton props or symbols unless they are part of a small source logo being replaced by the random chest badge.
Poster layout: square 1:1. In source-photo-first mode, keep the source photo as the full poster base and place a compact overlay group for collar options plus footer/contact where it does not cover the face, jersey artwork, player name/number, logo, or important garment details. For non-football or flat apparel references, use the full split catalog poster layout: left half shows one Vietnamese adult male football player wearing the converted kit, full body, confident catalog pose; right half shows front jersey mockup, back jersey mockup, solid-color shorts mockup, collar options, size row, and footer.
Background style: {background_style["prompt"]}. {contrast}
Background composition: for source-photo-first mode, keep the original background and lighting nearly unchanged and add only subtle local shadow/blur plates behind text if needed for readability. For split catalog mode, make the whole square feel like one cohesive poster, not two unrelated vertical halves; the left model area and right catalog area may have different contrast levels, but they should share the same gradient language, light direction, accent colors, and atmospheric depth.
Top title: in source-photo-first mode, omit the large title unless there is clean empty space; if used, make "Football 2026 Collection" a small premium corner or top-edge lockup that does not dominate the photo. In split catalog mode, show text exactly "Football 2026 Collection" centered at the top of the right half. No top logo or brand badge. Treat it as a custom sports-poster title lockup, not default typed text: use premium display lettering, confident tracking, subtle outline/shadow or metallic/ink texture when appropriate, and designer-level spacing. It should feel like football campaign typography, elegant and intentional, without dominating the product.
Selected worn collar: use exactly "{worn_collar["label"]}" ({worn_collar["slug"]}) for the highlighted collar choice. In source-photo-first mode, show this selected collar clearly in the collar-options overlay and only adjust the worn collar if it can be done subtly without changing the original model, shirt proportions, or natural neckline. In split catalog mode, apply the same selected collar to the football kit worn by the model and to the main front/back jersey mockups so the displayed product is consistent.
{COLLAR_RENDERING_RULES}
...
Chest logo: in split catalog mode, use the random chest logo reference as the left-chest team badge inspiration and make the right chest show jersey number "{jersey_number}". In source-photo-first mode, do not force a new chest logo or right-chest number if the source already has good branding/details.
Back jersey: in split catalog mode, show "TÊN CẦU THỦ" above a large number "{jersey_number}" and "TÊN ĐỘI BÓNG" below. In source-photo-first mode, keep any existing back text/number natural and do not rewrite it unless the source has clear empty jersey space and the edit remains photorealistic.
Shorts: in split catalog mode, shorts are one solid color. No pattern, no gradient, no decorative print, no stripes, no contrast side panels, no contrast waistband, no contrast hem trim, no piping, no colored cuffs. Choose a single color that fits the source kit palette. The only allowed visual marks on the shorts are the number "{jersey_number}" and natural fabric wrinkles/shadows. In source-photo-first mode, leave existing shorts unchanged unless a tiny number can be added naturally.
Collar options: include a polished strip titled exactly "TÙY CHỌN CỔ ÁO" with five cards: "Cổ tròn", "Cổ V viền", "Cổ V chéo", "Cổ V phối", "Cổ polo". Match the swatches to the current kit palette. In source-photo-first mode, make this strip compact and elegant, placed along the bottom edge or in an open side/corner area with a subtle translucent plate; visually emphasize "{worn_collar["label"]}" as the selected collar without making the other options disappear. In split catalog mode, center the entire collar-options group horizontally inside the right panel.
Size row: show boxes exactly "S" "M" "L" "XL" "2XL" "3XL" "4XL". In source-photo-first mode, include this only if there is room without clutter; otherwise omit the size row to keep the source photo clean. In split catalog mode, center the entire size row horizontally inside the right panel, aligned to the same center axis as the collar-options group.
Footer: show compact contact text "X24SPORT.VN | HOTLINE: 0989 353 247". In source-photo-first mode, use a slim bottom ribbon, small corner lockup, or floating text over a subtle shadow/blur plate. In split catalog mode, also show "XEM THÊM SẢN PHẨM".
Constraints: male Vietnamese model only; no female model; no pickleball paddle; no source-site watermark/logo; no top logo; no X24 logo at the top; all text must be readable. In split catalog mode, product mockups must stay prominent and visually balanced. In source-photo-first mode, the original photo must remain the hero and overlays must feel discreet, integrated, and secondary.
Avoid: default office-document font, plain typed-looking headline, flat AI clothing, plastic-smooth jersey, patterned shorts, gradient shorts, contrast-trim shorts, striped shorts, missing chest/shorts number in split catalog mode, missing random chest logo in split catalog mode, Brazil branding, distorted Vietnamese text, oversized footer, watermark, top logo, unbalanced shirt mockups, warped jersey proportions, unnecessary redrawing of already beautiful source football photos."""
