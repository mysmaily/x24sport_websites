...
export async function CatalogPageView({ page, search = '', heading = 'Toàn bộ mẫu áo bóng đá.', description = 'Chọn một mẫu làm điểm xuất phát rồi điều chỉnh màu sắc, logo, tên số và nội dung theo nhu cầu thực tế.', canonicalPath = '/shop/', breadcrumbLabel = 'Sản phẩm', categorySlug }: { page: number; search?: string; heading?: string; description?: string; canonicalPath?: string; breadcrumbLabel?: string; categorySlug?: string }) {
  ...
  return <div className="section-shell py-4 sm:py-8">
    ...
    <form action="/shop/" className="mt-4 grid max-w-3xl grid-cols-[auto_1fr_auto] overflow-hidden rounded-lg border border-slate-300 bg-white" role="search"><Search className="ml-3 self-center text-slate-500" size={17} /><label className="sr-only" htmlFor="catalog-q">Tìm mẫu áo</label><input className="min-h-11 min-w-0 px-3 text-sm outline-none" defaultValue={search} id="catalog-q" name="q" placeholder="Tên mẫu, mã hoặc màu sắc..." type="search" /><button className="bg-[#0b1220] px-4 text-sm font-black text-white hover:bg-brand">Tìm</button></form>
    <nav className="relative mt-3 flex items-center gap-2 rounded-xl border border-slate-200 bg-[#f8f6f2] p-2" aria-label="Lọc mẫu áo theo nhóm và từ khóa">
      ...
      <div className="flex min-w-0 flex-1 snap-x gap-1.5 overflow-x-auto overscroll-x-contain scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" data-catalog-type-strip>
        <Link aria-current={!categorySlug ? 'page' : undefined} className={`inline-flex min-h-10 shrink-0 snap-start items-center rounded-full border px-3 text-xs font-black transition ${!categorySlug ? 'border-[#0b1220] bg-[#0b1220] text-white' : 'border-slate-200 bg-white hover:border-brand hover:text-brand'}`} href="/shop/">Tất cả mẫu</Link>
        ...
        <div className="absolute right-0 top-full z-30 mt-2 grid w-[min(320px,calc(100vw-32px))] grid-cols-2 gap-1.5 rounded-xl border border-slate-200 bg-white p-2 shadow-[0_18px_50px_rgba(15,23,42,.18)]">
          <Link className="col-span-2 flex min-h-10 items-center rounded-lg px-3 text-xs font-black text-slate-700 hover:bg-slate-100" href="/shop/">Tất cả từ khóa</Link>
