...
const OPERATION = 'product-image-refresh-20260729'
...
const uploadMedia = async (token, product, filePath, checksum) => {
  ...
  form.append(
    ...
    JSON.stringify({
      tenant: TENANT_ID,
      alt: `${product.name} trên mẫu nam Việt Nam`,
      sourceSystem: 'manual-imagegen',
      sourceId,
      sourceUrl: `local://${TENANT_SLUG}/${OPERATION}/${basename(filePath)}`,
      sourceChecksum: checksum,
      searchTags: [{ value: 'football' }, { value: 'imagegen' }, { value: OPERATION }],
    }),
  )
...
const productGalleryIds = (product) => (product.gallery || []).map((item) => (typeof item === 'object' ? item.id : item)).filter(Boolean)

const main = async () => {
  const before = JSON.parse(await readFile(join(ROOT, 'page4-products-before.json'), 'utf8'))
  ...
  for (const [index, product] of before.docs.entries()) {
    ...
    const oldGallery = productGalleryIds(product)
    const sourceId = `${OPERATION}:${product.id}`
    const existing = await mediaForSource(token, sourceId)
    const plannedGallery = index < 5
      ? [existing?.id || '<new-media-id>', ...oldGallery]
      : [existing?.id || '<new-media-id>']

    if (!APPLY) {
      log.push({ productId: product.id, name: product.name, file: basename(filePath), oldGallery, plannedGallery })
      continue
    }

    const { media, reused } = await uploadMedia(token, product, filePath, checksum)
    const nextGallery = index < 5 ? [media.id, ...oldGallery] : [media.id]
    ...
    log.push({
      ...
      oldGallery,
      nextGallery,
      updatedAt: (updated.doc || updated).updatedAt,
    })
  }
