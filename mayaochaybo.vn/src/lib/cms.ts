...
export const getTenant = cache(async () => {
  const params = new URLSearchParams({ 'where[slug][equals]': TENANT_SLUG, limit: '1', depth: '0' })
...
export const getProductCategory = cache(async (slug: string) => {
  const tenant = await getTenant()
  const params = new URLSearchParams({
    'where[and][0][tenant][equals]': String(tenant.id), 'where[and][1][slug][equals]': slug,
    limit: '1', depth: '0',
  })
...
export const resolveCategoryPath = cache(async (legacyPath: string) => {
  const tenant = await getTenant()
  const params = new URLSearchParams({
    'where[and][0][tenant][equals]': String(tenant.id), 'where[and][1][legacyPath][equals]': legacyPath,
    limit: '1', depth: '0',
  })
  const result = await api<Paginated<ProductCategory>>(`/api/product-categories?${params}`)
  return result.docs[0] ?? null
...
export const resolveProductPath = cache(async (legacyPath: string) => {
  ...
  const params = new URLSearchParams({
    'where[and][0][tenant][equals]': String(tenant.id), 'where[and][1][legacyPath][equals]': legacyPath,
    'where[and][2][publicationStatus][equals]': 'publish', limit: '1', depth: '1',
  })
...
export const resolveContentPath = cache(async (legacyPath: string) => {
  ...
  const params = new URLSearchParams({
    'where[and][0][tenant][equals]': String(tenant.id), 'where[and][1][legacyPath][equals]': legacyPath,
    'where[and][2][publicationStatus][equals]': 'publish', limit: '1', depth: '0',
  })
  const result = await api<Paginated<WebContent>>(`/api/web-content?${params}`)
  return result.docs[0] ?? null
})

export async function getLatestPosts(limit = 3, page = 1) {
  const tenant = await getTenant()
  const params = new URLSearchParams({
    'where[and][0][tenant][equals]': String(tenant.id), 'where[and][1][kind][equals]': 'post',
...
export function productImages(product: Product): MediaImage[] {
  const migrated = (product.gallery || []).filter((item): item is MediaImage => typeof item === 'object' && Boolean(item?.url))
...
export async function getAllCanonicalRoutes() {
  ...
  const load = async <T>(collection: 'products' | 'web-content' | 'product-categories') => {
    ...
    let page = 1
    while (true) {
      const params = new URLSearchParams({ 'where[tenant][equals]': String(tenant.id), limit: '100', page: String(page), depth: '0' })
      if (collection !== 'product-categories') params.set('where[publicationStatus][equals]', 'publish')
  ...
  const [products, content, categories] = await Promise.all([
    load<Product>('products'), load<WebContent>('web-content'), load<ProductCategory>('product-categories'),
  ])
