...
When building, redesigning, or editing a public interface, treat mobile as a
primary customer experience unless the user explicitly scopes the work to
desktop only. X24Sport mayao tenants should be designed and verified on phones
because most shoppers browse there, but mobile changes must not degrade desktop
layout, brand seriousness, or the existing visual hierarchy.

- Start layout decisions from `390x844` and nearby mobile widths, then expand to
  tablet and desktop.
- On homepage and landing-page heroes, keep the primary shopping CTA easy to
  reach on mobile, but do not shrink the hero, trivialize typography, hide
  important context, or create oversized call-to-action blocks solely to force it
  above the fold.
...
- Verify mobile and desktop screenshots and rendered measurements before calling
  a UI change complete; one viewport passing is not enough for public-facing
  changes.
