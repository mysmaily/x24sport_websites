...
`logo-white-*`, chỉ random trong nhóm tương phản đúng: ngực sáng dùng
`logo-dark-*`, ngực tối/bão hòa dùng `logo-white-*`. Không tự dùng logo đầu tiên
trong asset và không lặp một logo cho toàn batch nếu script đã chọn khác. Trong
prompt mockup/sales, yêu cầu imagegen dùng đúng local reference đó và đóng lên
ngực áo như badge nhỏ đã in thật trên vải. Chỉ bỏ logo mẫu khi người dùng yêu cầu
