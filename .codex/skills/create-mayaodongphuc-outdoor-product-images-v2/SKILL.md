...
description: "Create the faster Mayaodongphuc outdoor/team-building product set from a supplied garment photo: one square ecommerce main, one integrated article catalog that itself serves as the distinct Version B scene, a validated product-handoff.json, and default publication through create-tenant-product. Use when the user supplies a uniform and invokes the v2 skill, or asks to reduce processing time for ảnh main, ảnh catalog, ảnh nhúng bài viết, poster giới thiệu áo dã ngoại, or the established Mayaodongphuc logo/hotline/product-feature treatment."
---

# Create Mayaodongphuc Outdoor Product Images V2

Produce two final publishing images by default—`main` and `catalog`—plus a validated `product-handoff.json`, then publish the product through `create-tenant-product` in the same task unless the user explicitly limits the request to image output. To reduce per-product processing time, generate the catalog directly as the distinct Version B scene instead of creating a separate clean Version B image first. Treat the supplied garment construction, colors and decorative design as the product identity, but replace its garment branding under the rules below. Treat the factory's stated product properties as authoritative marketing facts.
...
## Build Two Publishing Images Fast
...
The preferred v2 flow is one generation for main and one generation for the finished integrated catalog. Only create an intermediate clean Version B when the user explicitly requests it or when a correction workflow cannot preserve garment fidelity without a clean source.
...
- the catalog does not itself contain a distinct Version B scene;
...
For a default full set, create one product-specific directory under `generated/mayaodongphuc-outdoor-product-images-v2/` in the active workspace so all local skill outputs stay grouped together and out of Git while the exact-name manifest remains discoverable without overwriting another product:

- Directory: `generated/mayaodongphuc-outdoor-product-images-v2/mayaodongphuc-<product>/`
- Main: `generated/mayaodongphuc-outdoor-product-images-v2/mayaodongphuc-<product>/mayaodongphuc-<product>-main.png`
- Catalog: `generated/mayaodongphuc-outdoor-product-images-v2/mayaodongphuc-<product>/mayaodongphuc-<product>-catalog.png`
- Handoff: `generated/mayaodongphuc-outdoor-product-images-v2/mayaodongphuc-<product>/product-handoff.json`
