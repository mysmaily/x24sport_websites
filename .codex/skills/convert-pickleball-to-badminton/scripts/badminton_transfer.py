...
DEFAULT_ENV_FILE = Path("/Users/hoang/hacado/wordpress_websites/mayaocaulong.vn/.payload-api.env")
...
def global_converted_path(wave_dir: Path) -> Path:
    return wave_dir.parent / "pickleball-to-badminton-converted-sources.jsonl"
...
def fetch_store_products(api_url: str, limit: int | None = None, page_size: int = 100) -> list[dict[str, Any]]:
    ...
    while limit is None or len(products) < limit:
        ...
        req = urllib.request.Request(
            ...
            headers={"Accept": "application/json", "User-Agent": "badminton-transfer/1.2"},
        )
        try:
...
def fetch_payload_products(
    ...
    while limit is None or len(products) < limit:
        ...
        req = urllib.request.Request(
            ...
            headers={"Accept": "application/json", "User-Agent": "badminton-transfer/1.3"},
        )
...
def download_source_url(url: str, destination: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "badminton-transfer/1.2"})
...
def assert_responsible_product_payload(payload: dict[str, Any], paired: bool) -> None:
    ...
    if "áo cầu lông" not in name.casefold() or sku not in name:
...
def final_product_payload(record: dict[str, Any], media_ids: list[int], env: dict[str, str]) -> dict[str, Any]:
    ...
    name = f"Áo Cầu Lông {code} {color_title} – {facts['title_shape']}"
    design_copy = f"{gradient}"
    if pattern:
        design_copy += f", {pattern}"
    short = (
        f"Áo cầu lông đặt may {code} tông {color_copy}, {design_copy}; "
        f"{shape}, nhận in tên, số và logo đội theo yêu cầu."
    )
    if len(short) > 230:
        short = (
            f"Áo cầu lông đặt may {code} tông {color_copy}; "
            f"{shape}, nhận in tên số và logo đội."
        )
    paragraphs = [
        ...
        "X24 nhận tùy chỉnh màu nhận diện, in tên, số và logo cho câu lạc bộ, đội thi đấu hoặc nhóm phong trào theo nội dung khách hàng xác nhận.",
...
def pose_direction_for_record(record: dict[str, Any]) -> str:
    ...
    pose_family = seeded_choice(["straight-front", "female-back-look"], f"{key}:pose-family")
    camera = seeded_weighted_choice(
        [
            ("medium catalog crop from upper thigh upward", 50),
            ("medium-full catalog crop from knees upward", 35),
            ("wide full-body catalog framing from head to shoes", 15),
        ],
        f"{key}:camera",
    )
    if pose_family == "female-back-look":
        pose = (
            "Female-only back-look pose: the female turns three-quarter back so the rear garment panel is clearly visible, "
            "then looks back over one shoulder with an elegant youthful silhouette. The male must NEVER turn his back; "
            "keep him front-facing or three-quarter-front with his chest and shirt front readable. Do not add random "
            "logos, names, numbers, or invented graphics to the female shirt back; continue only the source garment pattern naturally."
        )
    else:
        pose = (
            "Straight/front catalog pose: both models stand or pause upright in a relaxed product-selling stance, with "
            "their garment fronts readable. Use a natural asymmetric weight shift and gaze variation rather than a stiff mirrored pose."
        )
    return (
...
def lighting_direction_for_record(record: dict[str, Any]) -> str:
    ...
    lighting = seeded_choice(
        [
            "soft open shade under the court roof with gentle daylight bounce on faces",
            "bright but diffused roof-filtered daylight with no direct sun on faces",
            "a soft warm sun patch brushes the models' faces from the side while the roof keeps the bodies and court evenly exposed",
        ],
        f"{key}:lighting",
    )
    return (
        f"Seeded lighting: {lighting}. Keep the outdoor covered-court background about 1 to 1.5 stops darker and "
        "less saturated than the models. Preserve sky and building detail; no clipped white areas, blown highlights, "
        "harsh flash, or bright background competing with the apparel."
    )
...
Default conversion strategy: full regeneration guided by the source garment. Use the provided final pickleball product image only as a garment-design reference, not as an edit target. Generate a new clean badminton apparel catalog photograph. Preserve the source garment facts: colorway ({colors}), gradient, pattern map, chest-mark placement, neckline, collar or sleeve construction, armhole coverage, seams, hems, fabric texture, folds, and how the print follows the body. Do not preserve pickleball paddles, paddle grip geometry, old overlays, or pickleball court cues.

Output requirement: Vietnamese badminton apparel catalog photo for product {code}, source variant: {source_variant_label}. Preserve this variant's visible garment construction so the two gallery images remain construction options of one SKU. For no-sleeve variants, preserve each model's exact neckline and collar construction from the source: the male may be round-neck sleeveless while the female may be sleeveless with a folded/standing collar. Do not convert a sleeveless folded-collar female top into a round-neck top. Use a modern badminton court or indoor/semi-outdoor badminton hall with realistic badminton court lines, suspended badminton net geometry, sports hall lighting, and subtle club background. A small or distant in-venue X24 Sport advertising banner is allowed when it looks naturally mounted and stays secondary. Do not show volleyball, pickleball, tennis, running, race, or old source-site cues.

Models and pose: use one fictional Vietnamese adult male and one fictional Vietnamese adult female together when the source contains a paired product image. Keep a relaxed catalog/product posture with garment fronts readable. Avoid jump smash, lunging action, overdramatic athletic poses, or motion blur. Use this seeded composition guidance: {pose_direction}
...
Badminton context requirements: do not put rackets in the models' hands. Both models should pose naturally with empty hands, with hands at sides, lightly behind the back, or relaxed near the waist. Make the badminton context unmistakable through the environment: regulation badminton court boundary lines, service lines, center lines, suspended badminton net with posts and clear open air below, a small shuttlecock tube and two white shuttlecocks on the floor near a sideline, and optionally a subtle distant X24 Sport Badminton Club or Sân cầu lông X24 sign. Keep floor props secondary and away from garment details.

Hand/equipment requirements: no handheld sports equipment. Reject rackets in hands, pickleball paddles, tennis rackets, balls in hands, random props, or hands shaped as if holding invisible equipment.

Style: premium Vietnamese badminton sportswear photography, clean bright skin, accurate anatomy and hands, apparel in sharp focus, background secondary and not cluttered. {lighting_direction} Visual priority must be: garment color/pattern and sewn construction first; female model and face second; badminton court/net/floor props third; venue last. Keep seams, neckline binding, armhole binding, fabric weave, folds, hem, and pattern flow sharper and more contrast-defined than the background.

Avoid: handheld rackets, pickleball court lines, pickleball paddles, tennis rackets, balls in hands, random sports props, oversized shuttlecocks, foreground ad text, net mesh reaching the floor, badminton net confused with a perimeter fence, missing open space below the net, volleyball context, running cues, source site names, garment color/pattern drift, collar or sleeve changes, flat pasted shirt print, broken collars, distorted hands, hands posed around invisible equipment, unreadable random text, harsh cutout artifacts, and any shopper-facing old-source branding."""
...
def process_record(record: dict[str, Any], wave_dir: Path, env: dict[str, str], client: PayloadClient | None, dry_run: bool) -> None:
    ...
    if record["status"] == "source_copied":
        ...
        for idx, variant in enumerate(source_variants_for_record(record), start=1):
            ...
            prompt_path = prompt_dir / f"{str(record['product_code']).lower()}-badminton-{idx}-{variant.get('kind') or 'variant'}.txt"
