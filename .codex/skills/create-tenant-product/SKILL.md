...
description: Create or update X24Sport Payload CMS products from product images with tenant-safe, idempotent REST publishing and fast public verification. Use when the user asks to đăng sản phẩm, tạo sản phẩm, upload product, import product, create product listing, or publish sportswear/catalog products.
---
...
- Root `/Users/hoang/hacado/x24sport-websites-gitlab-main/AGENTS.md`.
- Target domain profile: `/Users/hoang/hacado/x24sport-websites-gitlab-main/<domain>/AGENTS.md`.
- `/Users/hoang/hacado/x24sport-websites-gitlab-main/PAYLOAD-REST-API-GUIDE.md`.
...
- Read `references/master-catalog-distribution.md` only when the user explicitly requests distribution to another tenant or master catalog. Publishing to a specialist tenant does not authorize automatic cross-tenant copies.
- When an upstream `product-handoff.json` is supplied or discoverable beside the input images, read its producer-specific contract before using it. For V4 outdoor uniform outputs, read `/Users/hoang/hacado/x24sport-websites-gitlab-main/.codex/skills/create-outdoor-uniform-images-v4/references/product-handoff.md`; for legacy outputs, read `/Users/hoang/hacado/x24sport-websites-gitlab-main/.codex/skills/create-outdoor-uniform-images/references/product-handoff.md`.
- For `create-mayaodongphuc-outdoor-product-images`, read `/Users/hoang/hacado/x24sport-websites-gitlab-main/.codex/skills/create-mayaodongphuc-outdoor-product-images/references/product-handoff.md` and validate with that producer's `scripts/validate_product_handoff.py`. For `create-mayaodongphuc-outdoor-product-images-v2`, read `/Users/hoang/hacado/x24sport-websites-gitlab-main/.codex/skills/create-mayaodongphuc-outdoor-product-images-v2/references/product-handoff.md` and validate with its v2 validator. For `tao-anh-dong-phuc-lop-truong-hoc` or `tao-anh-dong-phuc-cong-ty-doanh-nghiep`, read the matching producer's `references/product-handoff.md` and validate with its `scripts/validate_product_handoff.py`. Do not send Mayaodongphuc producers through a legacy validator.
- For `tao-anh-ao-ngo-nghinh`, read `/Users/hoang/hacado/x24sport-websites-gitlab-main/.codex/skills/tao-anh-ao-ngo-nghinh/references/product-handoff.md` and validate with that producer's `scripts/validate_product_handoff.py --require-publishing-set`. New schema `1.1` supplies a marketing hero, one student lifestyle image, then a 500×500 WebP print preview; legacy schema `1.0` supplies hero plus preview. Its full PNG print master is source-only and must never be uploaded.
...
1. Confirm scope:
   ...
   - Distribution is out of scope unless the user explicitly names a target tenant. Never fan out a specialist product merely because it was published successfully.
   - A product created directly in a master tenant must not fan out again. Never use a master clone as a distribution source.
...
5. Upload media and create/update product:
   ...
   - For validated `tao-anh-ao-ngo-nghinh` handoffs, upload exactly `acceptedImages` in manifest order. Schema `1.1` order is marketing hero, student lifestyle as contextual image 1, then 500×500 print preview as contextual image 2; legacy schema `1.0` remains hero then preview. Re-encoding to WebP Q100 must preserve the preview's 500×500 dimensions. Never upload `sourceAssets.printMaster`, any archive path, or another PNG master.
...
6. Optionally distribute to an explicitly requested catalog:
   - Run this step only when the user names a target tenant. Invoke the centralized catalog-distribution workflow for `sourceTenant:sourceProductId -> targetTenant` after the source product exists.
   ...
   - Do not simulate distribution with a tenant REST account: a tenant account cannot grant cross-tenant media access. If the centralized distribution worker and ledger are unavailable, leave the verified source listing unchanged and report only the requested distribution as blocked.
...
7. Verify:
   ...
   - For a playful-shirt schema `1.1` handoff, verify gallery order is marketing, student lifestyle, then print preview; the uploaded preview remains 500×500 WebP; and exactly two contextual figures reuse the second and third media records. Keep the legacy schema `1.0` verification as marketing then preview with one contextual figure.
   - Run a sibling-tenant isolation query when doing batch or cross-tenant work.
   - When cross-tenant distribution was explicitly requested, verify each requested distribution record, source/target identity, target ownership, shared-media access, distinct target copy, and draft/public status. Report each target separately.
...
set +a
node /Users/hoang/hacado/x24sport-websites-gitlab-main/.codex/skills/create-tenant-product/scripts/upsert-product.mjs \
...
The helper requires the tenant key for both dry-run and apply so dry-run validates the real tenant, categories, media identity, and product identity. Never ask the user to type this key; load the profile-documented secret non-interactively. Its JSON result includes per-stage timing. Query-only identity checks and category-count updates run concurrently where safe; media processing stays ordered to prevent duplicate-upload races.
...
- For `tao-anh-ao-ngo-nghinh`, report the student-lifestyle and 500×500 print-preview media IDs/URLs and confirm the full-resolution PNG was not uploaded.
