...
2. Resolve the handoff or analyze from scratch:
   ...
   - For a validated Mayaodongphuc handoff with `publishingIntent.action=publish`, continue through REST mutation and fast public verification in the same task. Use quote-only commerce defaults, derive stable `sourceId` from `producerSkill + acceptedImages[0].sha256`, and query existing tenant products before allocating a SKU. Use the next available `MDP-DN-###` SKU for the original Mayaodongphuc producer. For `create-mayaodongphuc-outdoor-product-images-v2`, use a customer-safe SKU that avoids the legacy numeric batch range without exposing the workflow version, such as `MDP-DN-HH-001` for a hình học design. Never put `v2`, producer names, AI/process labels, or workflow internals in customer-facing SKU, name, slug, descriptions, badges, tags, captions, or metadata. A retry must update the existing product rather than consume another SKU.
