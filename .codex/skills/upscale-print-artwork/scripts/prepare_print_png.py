#!/usr/bin/env python3
"""Place an artwork PNG on a print-sized PNG canvas without distortion."""
...
try:
    ...
    raise SystemExit(
        "prepare_print_png.py needs Pillow. Install it in the active Python environment, "
...
def parser() -> argparse.ArgumentParser:
    command = argparse.ArgumentParser(
        description="Create a non-destructive print-sized PNG from a selected artwork."
    )
    command.add_argument("source", type=Path, help="Selected artwork image")
    command.add_argument("output", type=Path, help="New print PNG path")
