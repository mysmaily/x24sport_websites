...
- For tenant homepages with a visual hero above the fold, inspect the rendered
  production HTML for image preloads before editing. Preload the actual first
  hero/LCP image with the correct mobile and desktop `media` hints when it is not
  generated by `next/image`, keep that image eager/high-priority, and ensure
  product grids or other below-fold media are not marked `priority` in a way that
  emits competing `<link rel="preload" as="image">` entries before the hero.
- When a homepage LCP score looks image-bound, compare the Lighthouse LCP element
  with the tenant actually being tested. Local Lighthouse runs for dynamic tenant
  routes must send the correct host or otherwise verify the returned HTML belongs
  to the target tenant before using the score as evidence.
