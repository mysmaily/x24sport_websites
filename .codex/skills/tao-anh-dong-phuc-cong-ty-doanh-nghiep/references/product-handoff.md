...
`publishingIntent` cố định `tenantSlug: "mayaodongphuc"`, `domain: "mayaodongphuc.com.vn"`, `categorySlug: "dong-phuc-doanh-nghiep"`, `pricingMode: "quote-only"`, `isPurchasable: false`, `stockStatus: "instock"`, `currency: "VND"`; chỉ `action` thay đổi giữa `publish`, `draft`, `images-only`.
