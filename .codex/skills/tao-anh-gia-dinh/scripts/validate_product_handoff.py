#!/usr/bin/env python3
"""Validate family-shirt handoff structure, checksums, roles, and image dimensions."""
...
def main() -> None:
    ...
    required = {
        ...
        "productIdentity", "familyVariant", "sourceAssets", "acceptedImages", "artworkFacts",
        "audiences", "useCases", "unsupportedClaims", "suggestedCategories", "copySeeds",
    }
    missing = required - data.keys()
    if missing:
        fail(f"missing top-level fields: {sorted(missing)}")
    if data["schemaVersion"] != SCHEMA or data["producerSkill"] != PRODUCER:
    ...
    if categories != ["dong-phuc-gia-dinh"]:
        fail("publishingIntent.categorySlugs must be exactly ['dong-phuc-gia-dinh']")
    if any(not SLUG_RE.fullmatch(str(x)) for x in categories):
    ...
    family = data["familyVariant"]
    if not isinstance(family, dict):
        fail("familyVariant is required")
    generations = family.get("generations")
    cast_count = family.get("castCount")
    if generations == "three":
        if cast_count not in {5, 6, 7}:
            fail("three-generation familyVariant.castCount must be 5-7")
        if family.get("familyType") != "ba-the-he":
            fail("three-generation familyVariant.familyType must be ba-the-he")
    elif generations == "normal":
        if cast_count not in {3, 4, 5}:
            fail("normal familyVariant.castCount must be 3-5")
        if family.get("familyType") != "gia-dinh-hat-nhan":
            fail("normal familyVariant.familyType must be gia-dinh-hat-nhan")
    else:
        fail("familyVariant.generations must be normal or three")
    if not isinstance(family.get("roles"), list) or not family["roles"]:
        fail("familyVariant.roles must be a non-empty array")
    for key in ("scene", "action"):
        if not str(family.get(key, "")).strip():
            fail(f"familyVariant.{key} is required")
    if family.get("selection") != "stable-sha256":
        fail("familyVariant.selection must be stable-sha256")

    master_item = data["sourceAssets"].get("printMaster") if isinstance(data["sourceAssets"], dict) else None
    ...
    if not isinstance(images, list) or len(images) != 3:
        fail("acceptedImages must contain exactly 3 publishing images")
    paths: list[Path] = []
    roles: list[str] = []
    ...
    expected_roles = ["product hero", "content-inline family lifestyle", "print artwork preview"]
    ...
    if paths[1].name != f"{sku}-family-lifestyle.webp":
        fail("family lifestyle filename must be SKU-family-lifestyle.webp")
    if paths[2].name != f"{sku}-print-preview.webp":
        fail("preview filename must be SKU-print-preview.webp")

    for label, path, min_size in (
        ("marketing hero", paths[0], 1200),
        ("family lifestyle", paths[1], 1200),
    ):
        fmt, width, height = image_info(path)
        if fmt != "WEBP" or width != height or width < min_size:
            fail(f"{label} must be square WebP at least {min_size}px")
    preview_fmt, preview_width, preview_height = image_info(paths[2])
    ...
    if images[1]["productPlacement"].get("contentEmbed") is not True or images[1]["productPlacement"].get("contentOrder") != 1:
        fail("family lifestyle must be contentEmbed=true with contentOrder=1")
    if images[2]["productPlacement"].get("contentEmbed") is not True or images[2]["productPlacement"].get("contentOrder") != 2:
        fail("print preview must be contentEmbed=true with contentOrder=2")

    suggested = data["suggestedCategories"]
    if not any(item.get("slug") == "dong-phuc-gia-dinh" for item in suggested if isinstance(item, dict)):
        fail("suggestedCategories must include dong-phuc-gia-dinh")
    ...
    if args.require_publishing_set and requested != manifest_paths:
        fail("publishing-set validation must pass exactly the accepted images")

    print(
        f"PASS manifest={args.manifest.resolve()} producer={PRODUCER} "
        f"schema={SCHEMA} sku={sku} images=3 category=dong-phuc-gia-dinh"
    )
