#!/usr/bin/env python3
"""Add Mayaodongphuc logo, feature callouts, and optional hotline to one outdoor photo."""
...
def parse_hex_color(value: str) -> tuple[int, int, int]:
    ...
    if len(raw) != 6:
        raise argparse.ArgumentTypeError("color must be a 6-digit hex value, such as #740e26")
    try:
...
def main() -> int:
    ...
    parser.add_argument("--layout", choices=("row", "grid", "rail", "bottom-band"), default="bottom-band")
    parser.add_argument("--backdrop", choices=("diagonal", "gradient", "none"), default="none")
    parser.add_argument("--surface", choices=("paint", "none", "card"), default="none")
    parser.add_argument("--band-height", type=float, default=0.10, help="For bottom-band layout, band height as a fraction of the shorter image edge")
    parser.add_argument("--band-color", type=parse_hex_color, default=BURGUNDY[:3], help="For bottom-band layout, 6-digit garment-palette color")
    parser.add_argument("--band-alpha", type=int, default=188, help="For bottom-band layout, translucent band alpha 0..255")
    ...
    parser.add_argument("--fabric-title", default="Vải thoáng mát")
    parser.add_argument("--fabric-detail", default="Mềm nhẹ, dễ vận động")
    parser.add_argument("--design-title", default="Co giãn tốt")
    parser.add_argument("--design-detail", default="Thoải mái khi hoạt động")
    parser.add_argument("--durability-title", default="Form đồng bộ")
    parser.add_argument("--durability-detail", default="Gọn gàng cho đội nhóm")
    parser.add_argument("--printing-title", default="Hình in sắc nét")
    parser.add_argument("--printing-detail", default="Màu in rõ, nổi bật")
    ...
    if not 0.075 <= args.band_height <= 0.14:
        parser.error("--band-height must be between 0.075 and 0.14")
    if not 0 <= args.band_alpha <= 255:
        parser.error("--band-alpha must be between 0 and 255")
    ...
    if args.layout == "bottom-band":
        cluster_w = canvas_w - margin * 2
        cluster_h = round(unit * args.band_height)
        cluster_x = margin
        cluster_y = canvas_h - cluster_h
        logo_slot = 0
    ...
    if args.layout == "bottom-band" and args.auto_position:
        args.auto_position = False
    ...
    if args.layout == "bottom-band":
        band_y = canvas_h - cluster_h
        draw.rectangle((0, band_y, canvas_w, canvas_h), fill=(*args.band_color, args.band_alpha))
        draw.line((0, band_y, canvas_w, band_y), fill=(255, 255, 255, 70), width=max(1, round(unit * 0.0015)))
    elif args.backdrop == "gradient":
    ...
    if show_hotline:
        phone_h = round(unit * 0.04) if args.layout in ("row", "bottom-band") else round(unit * 0.043)
    ...
    cards_y = cluster_y if args.layout in ("row", "bottom-band") else cluster_y + header_h + (gap if header_h else 0)
    ...
    for index, (kind, title, detail) in enumerate(features):
        if args.layout in ("row", "bottom-band"):
        ...
        if args.layout == "bottom-band" and index > 0:
            separator_x = x - gap // 2
            draw.line(
                (separator_x, y + round(card_h * 0.22), separator_x, y + round(card_h * 0.82)),
                fill=(255, 255, 255, 76),
                width=max(1, round(unit * 0.001)),
            )
        ...
        if args.layout == "bottom-band":
            local_primary, local_secondary, local_stroke = (255, 255, 255, 255), (255, 255, 255, 218), (0, 0, 0, 0)
        else:
