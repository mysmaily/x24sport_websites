# x24sport.vn — Shared Frontend Tenant Profile
...
Generated from the latest `website_configs` record. Because `x24sport.vn` is a
merged `cms-frontend/` tenant, do not leave a generated `x24sport.vn/` folder in
the repository. If this legacy registry block must be refreshed, update the
marker block in this file and remove any generated folder before committing.
Local notes outside the markers are preserved.
...
- A display/layout/routing/metadata change is a **frontend code change**. Work in
  `cms-frontend/`, build locally, then deploy only the `cms-frontend` service.
...
- Retrieve the Cloudflare API token at runtime only with
  `ruby scripts/website_registry.rb cloudflare-token x24sport.vn` from the
  repository root. Never print it in handoff output or persist it in code/docs.
