...
const sourceSlug = 'mayaocaulong'
const targetSlug = 'x24sport'
const defaultCategorySlug = 'ao-cau-long'
...
function buildCopy(product: Doc) {
  ...
  const useCase = 'áo cầu lông, luyện tập, thi đấu phong trào, câu lạc bộ và đội nhóm'
  ...
  const shortDescription = `${name} do X24Sport bán và tư vấn đặt hàng áo cầu lông cho luyện tập, thi đấu, câu lạc bộ và đội nhóm.`
  const paragraphs = attributes.length ? [intro, `Thông tin nổi bật: ${attributes.join('; ')}.`, consult] : [intro, consult]
  const listItems = [
    'Phù hợp cho luyện tập, thi đấu cầu lông và đồng phục đội nhóm.',
  ...
  const metaDescription = `${name} mã ${sku || 'sản phẩm'} bán tại X24Sport cho cầu lông, đội nhóm và thiết kế theo yêu cầu. Tư vấn size và đặt hàng nhanh.`
...
async function run() {
  ...
  for (const source of products) {
    ...
    const targetCategory = defaultCategory
