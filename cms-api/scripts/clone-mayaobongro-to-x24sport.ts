...
const defaultCategorySlug = 'bong-ro'
...
function buildCopy(product: Doc) {
  ...
  const useCase = 'bộ quần áo bóng rổ, thi đấu, luyện tập, trường học, câu lạc bộ và đội nhóm'
  ...
  const shortDescription = `${name} do X24Sport bán và tư vấn đặt hàng đồ bóng rổ cho thi đấu, luyện tập, trường học, câu lạc bộ và đội nhóm.`
  const paragraphs = attributes.length ? [intro, `Thông tin nổi bật: ${attributes.join('; ')}.`, consult] : [intro, consult]
  const listItems = [
    'Phù hợp cho thi đấu bóng rổ, luyện tập và đồng phục đội nhóm.',
  ...
  const metaDescription = `${name} mã ${sku || 'sản phẩm'} bán tại X24Sport cho bóng rổ, đội nhóm và thiết kế theo yêu cầu. Tư vấn size và đặt hàng nhanh.`
