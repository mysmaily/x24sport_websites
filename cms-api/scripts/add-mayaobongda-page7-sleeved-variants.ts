...
const SOURCE_SYSTEM = 'mayaobongda-page7-sleeved-variants-20260730'
const operationDir = path.resolve(
  process.env.IMPORT_OPERATION_DIR ||
    '../mayaopickleball.vn/operations/mayaobongda-page7-sleeved-variants-20260730',
)
...
async function main() {
  ...
  const planned: any[] = []
  const touchedProductIds = inventory.map((item) => Number(item.targetProductId)).filter(Number.isFinite)
  const productsBefore = await payload.find({
    collection: 'products',
    depth: 2,
    limit: touchedProductIds.length,
    overrideAccess: true,
    where: { and: [{ tenant: { equals: tenantID } }, { id: { in: touchedProductIds } }] },
  })
  ...
  for (const item of inventory) {
    ...
    planned.push({
      ...
      slug: product.slug,
      currentGallery: Array.isArray(product.gallery) ? product.gallery.map(relationId).filter(Number.isFinite) : [],
  ...
  fs.writeFileSync(
    path.join(backupDir, `pre-sleeved-variants-backup-${new Date().toISOString().replace(/[:.]/g, '-')}.json`),
  ...
  for (const item of inventory) {
    ...
    const variantTags = [
      'có tay',
      item.variantSlug === 'co-tay-co-tron' ? 'cổ tròn' : 'cổ trụ',
      item.variantSlug === 'co-tay-co-tron' ? 'áo pickleball có tay cổ tròn' : 'áo pickleball có cổ trụ/cổ gập tay ngắn',
      item.metadata.style,
    ...
    if (!media) {
      media = await payload.create({
        ...
        data: {
          tenant: tenantID,
          alt: `${product.name} phiên bản ${item.metadata.garmentCut} trên sân pickleball`,
          searchTags: rows([
            ...rowValues(product.searchTags),
            ...variantTags,
            'ảnh người lớn nam nữ trên sân pickleball',
            'pickleball court lifestyle',
          ]),
          sourceSystem: SOURCE_SYSTEM,
          sourceId,
        ...
        file: {
          data: imageBuffer,
          mimetype: 'image/webp',
          name: `${product.slug}-${item.variantSlug}.webp`,
