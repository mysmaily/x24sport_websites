...
4. Generate two product-selling pickleball images by default: Vietnamese adult
   male and female models together in the same image, authentic pickleball court
   context, no running cues.
...
marks the record `analyzed`, and exits softly. Use Codex image generation
...
`skipped_duplicate`. An `analyzed` record without `new_product_id` is an
old/unclaimed prompt and is not safe for manual imagegen.

Status flow:

```text
source_discovered -> reserved -> source_downloaded -> analyzed
...
- Image A and Image B for one source product must show the same SKU, colorway,
  ...
  no-sleeve jerseys, with the male in a round neck and the female usually in a
  collared no-sleeve jersey. The female Image B variant may occasionally be
  round-neck no-sleeve too, but at a lower deterministic probability. Reject A/B
  pairs where the pattern design, stripe direction, color blocking, or accent
  placement differs beyond the required neckline/sleeve variation.
...
- Use one fictional Vietnamese adult male and one fictional Vietnamese adult
  female together in every generated image. Never generate a solo male image,
  solo female image, or single-model product photo for this workflow.
- Models must only stand or pause in relaxed product-selling/catalog poses while
  holding pickleball paddles. Do not create action shots, forehands, backhands,
  serves, split-step lunges, low ready-to-return stances, running, jumping, or
  any pose that looks like actively playing a point.
- Garments must look like real sewn sportswear worn by the models, not a flat
  graphic layer, Photoshop mockup, pasted print, decal, or texture stretched over
  a body. Collars, plackets, armholes, sleeve hems, side seams, shoulder seams,
  and waist hems must have physical thickness and stitching. The pattern must
  bend naturally with body curvature, fabric tension, folds, shadows, and
  perspective instead of looking like a flat rectangular print.
- Male shirts must not look cộc, cropped, or box-short. Keep a normal athletic
  jersey body length that extends comfortably below the waistband, with a
  believable hanging hem and some natural lower-torso drape.
- Do not render the shirt as dead flat single-color blocks. If a source area is
  visually simple or close to one color, reinterpret it into a tasteful tonal
  gradient in the same color family so the shirt still feels premium,
  intentional, and believable. Preserve the original palette direction, but add
  depth instead of leaving lifeless solid fills.
- When a source design contains noisy secondary accents that are not essential
  to the garment identity, it is acceptable to simplify them into a cleaner
  premium composition, as long as the main palette direction, major panel flow,
  and key line language remain recognizable. Do not add extra accent scribbles
  just to fill space.
- Fabric light response must feel photographic and physical: visible sun
  highlights, soft shadow falloff, seam depth, and natural wrinkle/fold
  transitions across chest, waist, shoulders, and side panels. Do not let the
  design read like a color slab painted onto the body.
- Do not leave the male torso reading stiff or vacuum-sealed. Preserve small
  natural wrinkle breaks and gentle fabric drape around the lower torso, waist,
  and hem so the shirt feels worn, soft, and real.
...
- Female face rendering should prioritize a naturally bright fair complexion,
  lively eyes, soft clean makeup, and a playful charming smile with visible
  natural dimples. Prefer a youthful Vietnamese catalog beauty feel: gentle
  jawline, slightly fuller soft cheeks, fresh white-bright skin, cute/nghich
  expression, and a warm "nang tho" mood rather than a severe, hardened,
  older-than-intended, blank, or over-defined face. The smile should feel
  ngộ nghĩnh and approachable, not stiff, forced, smug, or mannequin-like. Keep
  face, neck, arms, and legs color-consistent and realistic. Reject muddy, gray,
  sallow, overly tanned, plastic, filter-white, waxy, artificially sharpened,
  dull-eyed, tired, harsh-jawed, unsmiling, or face/body skin mismatch results.
- Use lightweight seeded scene/pose variation only. Do not compare against prior
  ...
  earlier product.
- Use seeded model placement and gaze variation. Male/female positions must be
  able to swap; do not always place the female on the right. Gaze direction
  should vary between looking at camera, camera-left, camera-right, and natural
  glances between models.
- Include occasional seeded female-only back-view or three-quarter-back
  compositions so the back design can be seen. The male must remain front or
  side-facing in these compositions. Keep back-view as a minority variant and at
  most one image in an A/B pair, so most product pairs still show clear
  front-facing selling views. Do not place random logos, `X24` marks, player
  names, or numbers on the back. Source-style graphics, pattern continuation, or
  a tasteful slogan may appear on the back when it looks intentional.
...
- Paddle grip is strict: every visible pickleball paddle must be physically held
  by a model's hand. The handle must pass through the palm or be wrapped by
  visible fingers, with believable contact, occlusion, and grip pressure. The
  hand must hold the paddle with a firm athletic grip, not a loose fingertip
  pinch, dangling prop hold, or delicate "holding a feather" gesture. Do not
  allow floating paddles, pasted paddles, paddles hovering in front of the body,
  handles ending beside the hand, or paddles attached to wrists/clothes without a
  real hand grip.
- Do not place random logos, `X24` marks, player names, or numbers on the back
  of any garment. This is especially important for female back-view shots.
  Source-style graphics, pattern continuation, or a tasteful slogan may appear
  on the back when it looks intentional.
...
- Image B neckline/sleeve rule: male must wear `áo cổ tròn, không có ống tay`
  with broad shoulders, shallow high armholes, and full side coverage. Female
  must wear a collared no-sleeve jersey with the same conservative armhole
  treatment. Removing sleeve tubes is the only change; do not convert either
  model into an `áo ba lỗ` silhouette.
...
- Image B requires no sleeve tubes on both models. Male is round neck. Female is
  normally collared no-sleeve unless the prompt explicitly selected the less
  frequent round-neck no-sleeve female variant.
...
- Reject deep tank tops, racerback/stringer cuts, narrow straps, oversized
  ...
  not `3 lỗ`.
- On male no-sleeve jerseys specifically, keep visibly more fabric at the front
  armpit and upper side seam, as if the edge were pulled outward roughly 2-3 cm
  from a buggy tank-top cut. Reject crescent-cut, hollowed-out, or inward-scooped
  male armholes.
- Reject male shirts whose body length stops too high, feels cropped, or looks
  stiff and cứng with no natural lower-torso wrinkles.
- Reject pasted-face artifacts, mismatched face/neck/body skin, broken jawline,
  stiff cutout hair, wrong head scale, dull/tired eyes, harsh mature face,
  forced smile, missing dimples when the female model is front-facing,
  distorted hands, floating or pasted paddles, paddle handles not actually held
  by fingers, loose fingertip paddle pinches, weak dangling paddle holds,
  feather-like paddle handling, running poses, race cues, and unrelated readable
  background text. Allow `X24 Sport` only as the required incidental
  storefront/signage text.
- Reject flat pasted shirt graphics, Photoshop-layer looks, decal-like prints,
  texture-wrap artifacts, rectangular patterns pasted onto torsos, missing seams,
  painted-on collars, or collars/armholes that do not physically connect to the
  garment.
- Reject shirts whose large panels look like lifeless solid paint fills without
  tonal variation, or whose lighting/folds are too weak to separate fabric from
  a pasted color block.
