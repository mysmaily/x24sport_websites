...
def infer_colors(filename: str, analysis: dict[str, Any] | None = None) -> list[str]:
    ...
    for needle, color in mapping:
        if needle in tokens and color not in colors:
            colors.append(color)
    if analysis:
...
HARD_IMAGE_COMPOSITION_RULE = (
    ...
    "while physically holding pickleball paddles by the handles. Each visible paddle handle must pass through a model's "
    "palm or be wrapped by visible fingers with believable hand contact, occlusion, and grip pressure. The hand must "
    "hold the paddle with a firm athletic grip, not a loose fingertip pinch, dangling prop hold, or delicate "
    "'holding a feather' gesture; no paddle may float, hover in front of the body, be pasted onto the image, or hang "
    "beside a hand without being gripped. "
    "Do not create a single-model image. Do not create action shots, forehands, "
...
FABRIC_REALISM_RULE = (
    ...
    "hem, and side panels instead of staying as a flat rectangular print. Avoid stiff mannequin-flat torsos: the shirt should "
    "show believable drape and small natural wrinkle breaks around the lower torso and waist."
)
...
LIGHTING_WRINKLE_RULE = (
    ...
    "depth, and natural wrinkle/fold transitions across the chest, waist, shoulders, side panels, and lower hem area. Do not "
    "let the garment read like a smooth painted color patch pasted onto the body."
)
...
def generate_prompt(record: dict[str, Any], slot: str, colors: list[str]) -> str:
    ...
    if slot == "a":
        variant = "Image A garment rule: both male and female wear short-sleeve collared pickleball shirts with true folded polo collars, clean short plackets, real sleeve tubes, broad athletic shoulders, and the same source-derived colorway/pattern map. Male shirts should keep a normal athletic jersey body length that reaches comfortably below the waistband, not cropped or visually short. Image A must not be sleeveless and must not use round necks."
...
Pose: {pose}. {staging} Camera/framing: {camera}. Product-selling posture, relaxed confidence, blank pickleball paddles required with no logos/text. Paddle grip rule: every paddle must be held by a real hand, with the handle visibly inside the palm or enclosed by fingers in a firm athletic grip; never let a paddle float, hover, appear pasted in front of the model, dangle from fingertips, or look delicately pinched like holding a feather. Apparel is the main focus. Use wide full-body framing only when selected by the camera instruction; otherwise keep the current medium product-selling crop. If a back-view moment is selected, only the female may turn back or three-quarter-back; keep the garment back design readable, avoid random back branding, and preserve realistic anatomy with enough face/head turn to feel natural.

Style: photorealistic commercial Vietnamese pickleball sportswear photography in soft natural outdoor daylight, with open shade or gentle morning sun, soft highlight rolloff, realistic ambient bounce light, and no harsh direct flash look. Skin and fabric should feel naturally lit by the environment rather than studio flash. Keep flattering clean bright skin tones, realistic anatomy/hands/fabric. Female face lighting should stay bright, fresh, fair, white-skinned, cute, smiling with natural dimples, and cheerful even in shade; avoid darkened or sallow facial rendering. Do not add official logo badge, bottom contact pill, color dots, or watermark; these are added later.

Avoid: flat pasted shirt graphic, Photoshop-layer look, decal-like print, texture wrap artifacts, rectangular pattern pasted onto torso, seams missing or painted on, collar floating above shirt, large dead flat one-color panels, lifeless solid fills with no tonal gradient, shirt lighting so weak that the garment reads like a color block stuck onto skin, harsh on-camera flash, blown specular highlights on skin, hard studio flash shadows, single model, solo male, solo female, action swing, forehand, backhand, serve, split-step, lunge, low ready-to-return stance, running pose, race bib, finish line, medals, old catalog frame, source logos/text, oversexualized pose, distorted hands, floating paddles, hovering paddles, pasted paddles, paddle handle not inside the hand, paddle handle ending beside fingers, loose fingertip paddle pinch, feather-like paddle handling, weak dangling paddle hold, paddle attached to clothing or wrist instead of gripped by fingers, random unreadable text, female face rendered dark, muddy, gray, sallow, overly tanned, waxy, artificially sharpened, mismatched against neck/body skin, too mature/hardened for a youthful catalog look, dull or tired eyes, harsh jawline, stiff/forced smile, smug expression, unsmiling female model, missing natural dimples on front-facing female model, mismatched Image A/B pattern design, alternate colorway between A/B, different pattern layout between A/B, unnecessary extra accent scribbles that clutter an otherwise simple premium design, zipper collar, mandarin collar, crew-neck zipper hybrid, round-neck placket hybrid, deep tank top, ao ba lo, undershirt silhouette, oversized armhole, exposed armpit, curved inward armpit scoop, hollowed-out male armhole, crescent-cut male armpit, low side opening, side edge bending inward at the armpit, or shoulder fabric eaten away into the neckline."""
...
def verify_public_batch(wave_dir: Path, source_keys: list[str], env: dict[str, str]) -> None:
    ...
    for key, record in records.items():
        ...
        product_id = int(record.get("new_product_id") or 0)
        category_contains_slug = slug in category_html
        category_verified_by = "public_html"
        if not category_contains_slug and product_id:
            product = product_by_id(product_id, env) or {}
            category_ids = {int(cat.get("id")) for cat in product.get("categories", []) if cat.get("id") is not None}
            category_contains_slug = 95 in category_ids
            category_verified_by = "rest_category_membership" if category_contains_slug else "public_html"
        batch_public = {
            "public_product_status": status,
            "category_contains_product_slug": category_contains_slug,
            "category_verified_by": category_verified_by,
    ...
    try:
        ...
        if record["status"] in {"source_downloaded", "analyzed", "prompts_ready"}:
            ...
            if image_provider == "manual-imagegen":
                ...
                return
            raise RuntimeError("Unsupported image provider; this workflow only supports manual-imagegen.")
...
def main() -> int:
    ...
    parser.add_argument("--image-provider", choices=["manual-imagegen"], default="manual-imagegen", help="Prepare prompts for Codex image generation and continue after manual outputs are added.")
