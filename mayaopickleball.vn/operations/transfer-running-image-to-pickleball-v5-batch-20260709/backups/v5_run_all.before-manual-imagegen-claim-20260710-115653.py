...
POSE_VARIANTS = [
    "standing side by side near the non-volley zone line, relaxed catalog posture, paddles held low",
    "walking slowly along the net after practice, natural smiles, apparel fully visible from thighs upward",
    "three-quarter stance with one model slightly forward and the other angled back, calm product-selling pose",
    "standing slightly apart with the net diagonally behind them, one blank paddle visible, no action swing",
    "waist-to-knee crop with both torsos clearly visible, court lines forming a clean diagonal background",
]
...
def generate_prompt(record: dict[str, Any], slot: str, colors: list[str]) -> str:
    ...
    variant = "both models wear the exact same product design and colorway for this SKU: coordinated collared no-sleeve pickleball jerseys with real folded polo collars, clean short plackets, broad shoulder panels, and no sewn sleeve tubes. The female jersey must be sleeveless but collared: no sleeve tube, real folded collar, clean short placket, shoulder fabric still covers the top of the shoulder and outer deltoid cap. This is a no-sleeve collared jersey, not a deep-cut tank top. The outer garment edge must form one clean, mostly straight vertical-to-diagonal line from the lower waist/side seam up to the shoulder cap, with no inward scoop, no curved bite at the armpit, and no side edge bending inward. No exposed armpit, no racerback cut, no stringer tank, no narrow shoulder straps, no armhole scooped inward toward the chest or back"
    neckline = "Both male and female jerseys must have true folded polo collars with short clean plackets. Do not create zipper collars, mandarin tabs, faux plackets, broken hybrid collars, or collarless tank tops."
...
Product consistency rule: Image A and Image B for this product must show the same SKU, same garment cut, same collar style, same colorway, same pattern map, same accent placement, and same small X24 chest mark. Only the camera framing, pose, and background may change. Do not invent a second shirt design, alternate color placement, different stripe direction, different collar, or different sleeve/armhole construction between A and B.

{identity}

Garment style: {variant}. Both wear matching black pickleball shorts/skort. Authentic court-sport fabric, realistic folds, relaxed athletic torso. Armhole construction must look feasible for a Vietnamese apparel factory: shoulder seam stays near the natural shoulder edge, shoulder fabric remains wide and stable, the side panel/armhole edge runs as one clean straight line from waist to shoulder, the arm opening is high and minimal, and the side seam rises high enough that the underarm and side torso are not exposed. Add only a small X24 mark on visible shirt fronts; no other readable garment text.

Neckline and armhole quality rule: {neckline} Do not create a deep-cut tank top, bodybuilding singlet, loose armhole, cutaway shoulder, curved underarm scoop, inward armpit bite, or exposed-armpit design.
...
Pose: {pose}. Camera/framing: {camera}. Product-selling posture, relaxed confidence, blank pickleball paddle optional with no logos/text. Apparel is the main focus. Use wide full-body framing only when selected by the camera instruction; otherwise keep the current medium product-selling crop.
...
Avoid: running pose, race bib, finish line, medals, old catalog frame, source logos/text, oversexualized pose, distorted hands, random unreadable text, mismatched Image A/B garment design, alternate colorway between A/B, different pattern layout between A/B, zipper collar, mandarin collar, crew-neck zipper hybrid, round-neck placket hybrid, collarless tank top, deep tank top, oversized armhole, exposed armpit, curved inward armpit scoop, side edge bending inward at the armpit, shoulder fabric eaten away into the neckline."""
...
def product_payload(record: dict[str, Any], media_ids: list[int], media_urls: list[str], tag_ids: list[int], colors: list[str]) -> dict[str, Any]:
    ...
    short = f"Bộ quần áo pickleball {code} nổi bật với tông {color_copy}, form áo không tay có cổ vai rộng cho nam nữ, dễ mặc cho luyện tập, giao lưu câu lạc bộ hoặc đồng phục đội nhóm."
    desc = f"""
...
<figure><img src="{media_urls[0]}" alt="Bộ quần áo pickleball {code} {color_copy} áo không tay có cổ" /></figure>
<p>Form áo không tay có cổ được tối ưu cho các động tác xoay người, vươn tay và di chuyển ngang. Phần cổ gập tạo cảm giác lịch sự cho câu lạc bộ, trong khi rập vai rộng không ống tay giúp người chơi vận động thoải mái mà vẫn gọn gàng, dễ sản xuất.</p>
<p>Hai hình ảnh sản phẩm thể hiện cùng một mẫu thiết kế với cùng tông màu, cùng bố cục họa tiết và cùng form áo không tay có cổ. Các mảng màu và họa tiết trên thân áo giúp đội nhóm dễ nhận diện, lên ảnh đẹp và phù hợp với quần short hoặc chân váy thể thao màu đen.</p>
<figure><img src="{media_urls[1]}" alt="Bộ quần áo pickleball {code} {color_copy} cùng mẫu áo không tay có cổ" /></figure>
...
""".strip()
    payload = {
        ...
        "images": [
            {"id": media_ids[0], "position": 0, "alt": f"Bộ quần áo pickleball {code} {color_copy} áo không tay có cổ"},
            {"id": media_ids[1], "position": 1, "alt": f"Bộ quần áo pickleball {code} {color_copy} cùng mẫu áo không tay có cổ"},
        ],
        "meta_data": transfer_meta(record, "completed") + [
            {"key": "_yoast_wpseo_focuskw", "value": f"bộ quần áo pickleball {color_copy}"},
            {"key": "_yoast_wpseo_metadesc", "value": f"Bộ quần áo pickleball {code} tông {color_copy}, form áo không tay có cổ vai rộng cho nam nữ, thoáng nhẹ, phù hợp câu lạc bộ và đồng phục đội nhóm."},
            ...
            {"key": "_thumbnail_id", "value": str(media_ids[0])},
        ],
    }
