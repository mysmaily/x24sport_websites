...
const jobs = JSON.parse(await readFile(path.join(root, jobsFile), 'utf8'));
...
async function runJob(job) {
  ...
  form.append('image', new Blob([sourceBytes], { type: mimeType }), path.basename(sourcePath));
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`${job.id} ${job.identity}: HTTP ${response.status} ${text.slice(0, 500)}`);
    }

    const json = JSON.parse(text);
    const b64 = json.data?.[0]?.b64_json;
    if (!b64) {
      throw new Error(`${job.id} ${job.identity}: response did not include b64_json`);
    }
...
async function worker(queue, results) {
  while (queue.length) {
    ...
    try {
      ...
      results.push({ ok: false, id: job.id, product_id: job.product_id, error: error.message });
...
await writeFile(
  path.join(root, 'generation-results.json'),
  JSON.stringify({ model, concurrency, jobsFile, results }, null, 2)
);
