#!/usr/bin/env python3
"""Manage resumable V5 batch migration manifests."""
...
def canonical_record_index(records: list[dict[str, Any]], source_product_key: str) -> int | None:
    matches = [
        (idx, record)
    ...
    pool.sort(
        key=lambda item: (
            STATUS_ORDER.get(str(item[1].get("status")), -1),
...
def cmd_next(args: argparse.Namespace) -> int:
    ...
    for record in records:
        ...
        if not key or record.get("status") in TERMINAL_STATUSES:
            continue
        idx = canonical_record_index(records, key)
        if idx is None:
            continue
        latest_pending[key] = records[idx]
    pending = list(latest_pending.values())
...
def cmd_mark(args: argparse.Namespace) -> int:
    ...
    with manifest_lock(args.wave_dir):
        ...
        target_index = canonical_record_index(records, args.source_product_key)
