...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const audiencePaths = new Set<string>(BASKETBALL_AUDIENCES.map((audience) => audience.path))
  return [
    ...
    ...content.filter((item) => item.legacyPath !== '/' && item.legacyPath !== '/mau-ao-bong-ro/' && !audiencePaths.has(item.legacyPath) && !isRetiredPath(item.legacyPath)).map((item) => ({ url: canonical(item.legacyPath), lastModified: item.sourceModifiedAt || undefined, changeFrequency: 'monthly' as const, priority: 0.7 })),
  ]
}
