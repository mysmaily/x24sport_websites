...
$batchRoot = $argv[2] ?? '/var/www/mayaobongro.vn/wp-content/uploads/codex-ops/running-to-basketball-db-text-hits-20260708';
...
{
    $path = rtrim($batchRoot, '/') . '/manifest.json';
...
foreach (pages56_load_manifest($batchRoot) as $record) {
    ...
    $backupPath = $backupDir . '/product-' . $productId . '-before-db-text-hits-conversion.json';
    ...
    $elementaryPath = rtrim($batchRoot, '/') . '/final/' . $codeSlug . '-ao-bong-ro-tieu-hoc.webp';
    $highSchoolPath = rtrim($batchRoot, '/') . '/final/' . $codeSlug . '-ao-bong-ro-thpt.webp';
    $design = pages56_design_name($record);

    $elementaryMediaId = pages56_ensure_media(
        $elementaryPath,
        $codeSlug . '-tieu-hoc-' . $productId . '-db-text-hits',
    ...
    $highSchoolMediaId = pages56_ensure_media(
        $highSchoolPath,
        $codeSlug . '-thpt-' . $productId . '-db-text-hits',
