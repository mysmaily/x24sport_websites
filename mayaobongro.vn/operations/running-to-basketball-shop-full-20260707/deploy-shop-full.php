...
$batchRoot = $argv[2] ?? '/var/www/mayaobongro.vn/wp-content/uploads/codex-ops/running-to-basketball-shop-full-20260707';
...
foreach (pages56_load_manifest($batchRoot) as $record) {
    ...
    $backupPath = $backupDir . '/product-' . $productId . '-before-shop-full-conversion.json';
    ...
    $elementaryMediaId = pages56_ensure_media(
        $elementaryPath,
        $codeSlug . '-tieu-hoc-' . $productId . '-shop-full',
    ...
    $highSchoolMediaId = pages56_ensure_media(
        $highSchoolPath,
        $codeSlug . '-thpt-' . $productId . '-shop-full',
