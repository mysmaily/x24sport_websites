...
function mayaobongro_linked_editions_activate(): void
{
    $audiences = [
        [
            ...
            'description' => 'Áo bóng rổ cho học sinh tiểu học, trẻ em và các đội bóng rổ thiếu nhi.',
        ],
        [
            'name' => 'Áo Bóng Rổ Người Lớn',
            'slug' => 'ao-bong-ro-nguoi-lon',
            'description' => 'Áo bóng rổ cho học sinh trung học, người lớn, đội tuyển trường và câu lạc bộ.',
        ],
    ];

    foreach ($audiences as $audience) {
        $existing = get_term_by('slug', $audience['slug'], 'product_tag');
        if ($existing && !is_wp_error($existing)) {
            $updated = wp_update_term((int) $existing->term_id, 'product_tag', [
                'name' => $audience['name'],
        ...
        $created = wp_insert_term($audience['name'], 'product_tag', [
            'slug' => $audience['slug'],
            'description' => $audience['description'],
        ]);
...
function mayaobongro_linked_editions_add_candidate_terms(
    array &$candidates,
    mixed $terms,
    string $taxonomy,
    string $field
    ...
    foreach ($values as $termValue) {
        if ($field === 'term_id' || $field === 'id') {
            $term = get_term((int) $termValue, $taxonomy);
        ...
        if (is_scalar($termValue)) {
            $candidates[] = (string) $termValue;
        }
    }
}

function mayaobongro_linked_editions_extract_audience_from_query(WP_Query $query): ?string
{
    $candidates = [];
    $queriedObject = $query->queried_object ?? null;
    if (
        $queriedObject instanceof WP_Term
        && in_array($queriedObject->taxonomy, ['product_tag', 'product_cat'], true)
    ) {
        $candidates[] = $queriedObject->slug;
        $candidates[] = $queriedObject->name;
    }

    foreach (['mayaobongro_audience', 'product_tag', 'tag', 'pagename', 'name'] as $queryVar) {
    ...
    $taxQuery = $query->tax_query instanceof WP_Tax_Query
        ? $query->tax_query->queries
        : (is_array($query->get('tax_query')) ? $query->get('tax_query') : []);
    ...
    foreach ($taxQuery as $clause) {
        ...
        mayaobongro_linked_editions_add_candidate_terms(
            $candidates,
            $clause['terms'] ?? null,
            $taxonomy,
            (string) ($clause['field'] ?? 'term_id')
        );
    }

    $level = mayaobongro_linked_editions_resolve_unique_audience_level($candidates);
    if ($level) {
        return $level;
    }

    $pageId = (int) $query->get('page_id');
    if ($pageId > 0) {
        $page = get_post($pageId);
        if ($page instanceof WP_Post) {
            return mayaobongro_linked_editions_resolve_unique_audience_level([
                $page->post_name,
                $page->post_title,
            ]);
        }
...
function mayaobongro_linked_editions_get_active_loop_audience_level(): ?string
{
    static $stack = [];

    if (func_num_args() === 2) {
        $action = (string) func_get_arg(0);
        ...
        if ($action === 'push') {
            $level = mayaobongro_linked_editions_extract_audience_from_query($query);
            if ($level) {
                $stack[$id] = $level;
            }
            return $level;
...
function mayaobongro_linked_editions_track_loop_audience_start(WP_Query $query): void
{
    if (is_admin() || wp_doing_ajax()) {
        return;
    }

    mayaobongro_linked_editions_get_active_loop_audience_level('push', $query);
}
add_action('loop_start', 'mayaobongro_linked_editions_track_loop_audience_start', 5);

function mayaobongro_linked_editions_track_loop_audience_end(WP_Query $query): void
{
    if (is_admin() || wp_doing_ajax()) {
        return;
    }

    mayaobongro_linked_editions_get_active_loop_audience_level('pop', $query);
}
add_action('loop_end', 'mayaobongro_linked_editions_track_loop_audience_end', 5);

function mayaobongro_linked_editions_get_archive_level_from_query(): ?string
{
    if (is_admin() || wp_doing_ajax()) {
        return null;
    }

    $activeLoopLevel = mayaobongro_linked_editions_get_active_loop_audience_level();
    if ($activeLoopLevel) {
        return $activeLoopLevel;
    }

    $query = $GLOBALS['wp_query'] ?? null;
    if ($query instanceof WP_Query) {
        $resolvedLevel = mayaobongro_linked_editions_extract_audience_from_query($query);
        if ($resolvedLevel) {
...
function mayaobongro_linked_editions_get_contextual_archive_thumbnail_id(
    int $productId,
    int $fallbackImageId
): int {
    if (!mayaobongro_linked_editions_is_age_gallery_product($productId)) {
        return $fallbackImageId;
    }

    $level = mayaobongro_linked_editions_get_archive_level_from_query();
    if (!$level) {
        return $fallbackImageId;
    }

    return mayaobongro_linked_editions_pick_archive_image_id(
        $level,
        [
            'tieu-hoc' => (int) get_post_meta($productId, MAYAOBONGRO_AGE_IMAGE_TIEU_HOC_META, true),
            'trung-hoc' => (int) get_post_meta($productId, MAYAOBONGRO_AGE_IMAGE_THPT_META, true),
        ],
        $fallbackImageId
    );
}

function mayaobongro_linked_editions_filter_post_thumbnail_id($thumbnailId, $post): int
{
    $postId = 0;
    ...
    if ($postId < 1 || get_post_type($postId) !== 'product') {
        return (int) $thumbnailId;
    }

    return mayaobongro_linked_editions_get_contextual_archive_thumbnail_id(
        $postId,
        (int) $thumbnailId
    );
}
add_filter('post_thumbnail_id', 'mayaobongro_linked_editions_filter_post_thumbnail_id', 20, 2);

function mayaobongro_linked_editions_filter_product_image_id($imageId, $product): int
{
...
add_filter(
    ...
    'mayaobongro_linked_editions_filter_product_image_id',
    20,
    2
);
...
function mayaobongro_linked_editions_get_age_gallery_html(?int $productId = null): string
{
    ...
    if (!mayaobongro_linked_editions_should_render_age_fit_box(
        get_post_meta($productId, MAYAOBONGRO_AGE_GALLERY_MODEL_META, true)
    )) {
        return '';
    }
...
function mayaobongro_linked_editions_product_summary_proxy(): string
{
    ...
    <section class="x24-product-summary mbro-product-summary" aria-label="Thông tin sản phẩm">
        ...
        <p class="x24-product-subtitle">
            Bộ bóng rổ thiết kế riêng với áo và quần phối họa tiết đồng bộ, form thoáng cho tập luyện và thi đấu.
        </p>
...
function mayaobongro_linked_editions_size_guide_proxy(): string
{
    ...
    $sizeGuideLabel = $isAgeGallery ? 'Chất liệu & form bóng rổ' : 'Form bóng rổ theo cấp học';
    $headingText = $isAgeGallery
        ? 'Tư vấn size và chất liệu theo đội'
        : 'Chọn size cho ' . ($level === 'tieu-hoc' ? 'học sinh lớp 4–5' : 'học sinh lớp 11–12');
    $introText = $isAgeGallery
        ? 'Gửi chiều cao, cân nặng và yêu cầu chất liệu để xưởng tư vấn form áo/quần phù hợp.'
        : 'Gửi chiều cao và cân nặng từng học sinh để xưởng lên danh sách size chính xác.';
    ...
    <section class="x24-size-guide x24-size-guide--full mbro-size-guide" aria-labelledby="mbro-size-guide-title">
        <div class="x24-size-guide__heading">
            <div>
                <p class="x24-size-guide__label"><?php echo esc_html($sizeGuideLabel); ?></p>
                <h2 id="mbro-size-guide-title"><?php echo esc_html($headingText); ?></h2>
            </div>
        ...
        <p class="x24-size-guide__note">
            Liên hệ Zalo 0989.353.247 để nhận bảng size và tư vấn chất liệu phù hợp.
        </p>
    </section>
    <?php
