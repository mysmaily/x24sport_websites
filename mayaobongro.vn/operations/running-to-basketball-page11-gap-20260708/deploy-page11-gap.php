...
$batchRoot = $argv[2] ?? '/var/www/mayaobongro.vn/wp-content/uploads/codex-ops/running-to-basketball-page11-gap-20260708';
...
foreach (pages56_load_manifest($batchRoot) as $record) {
    ...
    $backupPath = $backupDir . '/product-' . $productId . '-before-page11-gap-conversion.json';
    ...
    $elementaryMediaId = pages56_ensure_media(
        $elementaryPath,
        $codeSlug . '-tieu-hoc-' . $productId . '-page11-gap',
    ...
    $highSchoolMediaId = pages56_ensure_media(
        $highSchoolPath,
        $codeSlug . '-thpt-' . $productId . '-page11-gap',
