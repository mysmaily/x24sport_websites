...
const config = JSON.parse(await readFile(path.join(root, 'product-config.json'), 'utf8'));
...
const product = await fetchJson(`https://mayaobongro.vn/wp-json/wp/v2/product/${config.product_id}?_fields=id,slug,title,link,featured_media,product_cat,categories,content,excerpt,yoast_head_json`);
const productCategories = product.categories || product.product_cat || [];
const htmlResponse = await fetch(`https://mayaobongro.vn/${config.slug}/?cb=${Date.now()}`);
...
const oldHead = await head(`https://mayaobongro.vn/${config.old_slug}/`);
...
if (product.slug !== config.slug) throw new Error(`Slug mismatch: ${product.slug}`);
...
if (oldHead.location !== `https://mayaobongro.vn/${config.slug}/`) throw new Error(`Old URL redirects to ${oldHead.location}`);
