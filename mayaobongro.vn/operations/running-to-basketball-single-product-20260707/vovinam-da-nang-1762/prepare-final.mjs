...
const swatches = [
  { label: 'Trắng', fill: '#ffffff', text: '#2d3748' },
  { label: 'Đen', fill: '#111111', text: '#ffffff' },
  { label: 'Hồng', fill: '#ff5fa2', text: '#ffffff' },
  { label: 'Đỏ', fill: '#e11d2e', text: '#ffffff' },
  { label: 'Vàng', fill: '#ffd447', text: '#1f2937' },
  { label: 'Xanh Blue', fill: '#1677ff', text: '#ffffff' },
  { label: 'Green', fill: '#18a957', text: '#ffffff' },
  { label: '+', fill: '#f8fafc', text: '#111827' },
];
...
async function prepareLogo(width) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (min > 248) {
      ...
      data[i + 3] = Math.round(data[i + 3] * ((248 - min) / 13));
    }
  }
...
function logoBacking(width, logoWidth, logoHeight) {
  ...
  return Buffer.from(`
    <svg width="${boxWidth}" height="${boxHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="4" y="4" width="${boxWidth - 8}" height="${boxHeight - 8}" rx="${radius}" fill="rgba(255,255,255,0.18)" filter="url(#soft)"/>
      <rect x="6" y="6" width="${boxWidth - 12}" height="${boxHeight - 12}" rx="${radius}" fill="rgba(255,255,255,0.08)"/>
    </svg>
  `);
}
...
function swatchOverlay(width, height, side = 'right') {
  ...
  const items = swatches.map((swatch) => {
    ...
    const plus = swatch.label === '+'
      ...
      : '';
    const out = `
      <circle cx="${circleX}" cy="${circleY}" r="${r}" fill="${swatch.fill}" stroke="rgba(255,255,255,0.92)" stroke-width="2.4"/>
      ${plus}
    `;
    cursor += r * 2 + gap;
    return out;
...
for (const job of jobs) {
  ...
  await sharp(input)
    ...
    .composite([
      {
        input: logoBacking(1000, logoMeta.width, logoMeta.height),
        top: logoTop - pad,
        left: logoLeft - pad,
        blend: 'over',
      },
