...
async function runJob(job) {
  ...
  if (await exists(outputPath)) {
    return { id: job.id, audience: job.audience, output: job.generated, skipped: true };
  }
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (!b64) throw new Error(`${job.id}: response did not include b64_json`);
...
async function worker(queue, results) {
  while (queue.length) {
    ...
    try {
      ...
      console.log(JSON.stringify({ ok: true, id: job.id, output: job.generated, skipped: result.skipped || false }));
    } catch (error) {
